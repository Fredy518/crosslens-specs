...
4. `in`/`not_in` 的 value 为 `list[float]`，判定输入值是否（不）为列表成员
5. 输入为 null 时不匹配任何规则，返回 `default_output`
6. `test_cases` 为强制项——每条规则表必须附带至少覆盖所有 rule_id 的边界测试
7. Derived Metric 的 confidence 继承所有输入 metric 的最小 confidence（`confidence_rule = "inherit_min"`）
