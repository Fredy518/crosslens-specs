...
**状态：** Approved
**项目名称：** crosslens
**依赖文档：** SPEC-001 v0.4；SPEC-003 v0.3.4；SPEC-004 v0.2.4
...
> **合并方向声明（v0.1.6，v0.2.8 修正）：** Playbook Evaluation Report 生成 `recommended_decision_bounds`，由 Resolved Decision Bounds 进一步合并 Guardrail、Validation 等约束。合并规则中，Guardrail 和 Validation 只能进一步收窄 `allowed_actions`（移除动作或降低 confidence），不得恢复已被 Playbook 移除的动作。`confidence_cap` 下调采用叠加语义——各来源（Playbook、Guardrail、Validation）的下调值依次累加，取最终结果（例如 Playbook 下调 0.05、Guardrail 再下调 0.05 → 累计下调 0.10），而非取各来源最大下调值。最终 cap 的下限检查在 Resolved Decision Bounds 阶段执行；完整合并规则由 SPEC-003 和 SPEC-009 定义。
...
**`not_applicable` 子规则处理（v0.1.8）：** 子规则 status = `not_applicable`（如 §33.1 `growth_capex_flag` 触发的例外）时，无论 `condition_logic` 如何，均跳过该子规则按剩余子规则计算。**所有子规则均为 `not_applicable` 时，整体 Constraint status = `not_applicable`（`all` 模式下不等于 `pass`——vacuous pass 在投资风控语境下应保守处理；`any` 模式下同理）**。此规则与 §22.3 规则 7/8 及 §23.2 规则 9 对齐。
...
| `passed_with_caution` | 无 Hard `fail` 或 unresolved status，且 ≥2 个 Soft Constraint fail；允许所有 Hard 均为 `not_applicable`，但必须在 Decision Trace 标注。`recommended_decision_bounds` 同 `passed`，`confidence_cap` 下调 0.05 |
...
> **`overall_result` 决策树（v0.1.7，v0.2.8 guard 显式化）：** 第 1-4 条域级门控通过后执行 Constraint 决策树；§8.4 第 5 条由本决策树中的 Hard status 检查实现。以下条件按优先级从高到低匹配，第一个满足即停止。`stale_data`、`insufficient_data`、`partial` 和 `error` 均不视为 `pass`。

| 规则 | 条件 | overall_result |
|---|---|---|
| 1 | 任一 Soft Constraint fail 含 `require_human_review`（优先） | `requires_human_review` |
| 2 | 任一 Hard Constraint fail（`block_new_position`） | `not_passed_for_new_buy` |
| 3 | 无 Hard fail，且任一 Hard status ∈ `{partial, insufficient_data, stale_data, error}` | `need_more_data` |
| 4 | **至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 Soft fail = 0 | `passed` |
| 5 | **至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 `0 < Soft fail < 2` | `partially_passed` |
| 6 | **至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 Soft fail ≥ 2 | `passed_with_caution` |
| 7 | **所有 Hard status = `not_applicable` 且无一实际 `pass`**，Soft fail < 2 | `partially_passed` |
| 8 | **所有 Hard status = `not_applicable` 且无一实际 `pass`**，Soft fail ≥ 2 | `passed_with_caution` |

规则 4 与规则 7/8 的 guard 互斥，不得依赖括号注释或实现者自行排除。规则 7/8 不阻断动作边界，但 Decision Trace 必须说明“本次没有实际 Hard Constraint 被验证”。
...
5. 至少一条 Hard Constraint 实际 `pass`、其余 Hard 为 `pass` 或 `not_applicable`，且 Soft Constraint fail ≥ 2 → `overall_result = passed_with_caution`（v0.2.8 guard 显式化）；
...
> **`confidence_cap` 下调幅度与阶段归属（v0.2.8）：** `passed_with_caution` 在 Playbook Evaluation Report 的 `confidence_cap_adjustments` 中记录默认绝对值下调 0.05；该幅度与 Soft Constraint fail 阈值（2）均可由 Run Config 配置。Playbook Evaluation 阶段不检查多来源叠加后的最终 cap，也不因 cap 下限改写自身 `overall_result`。Resolved Decision Bounds 合并 Playbook、Guardrail、Validation 等全部调整后，若最终 `confidence_cap < 0.5`，必须设置 `resolved_decision_bounds.requires_human_review = true`，并在 `confidence_cap_reason` 记录阈值触发原因。Decision Trace 同时保留 `playbook_evaluation_report.overall_result` 与 `resolved_decision_bounds.requires_human_review`；不得生成第二个或相互矛盾的 `overall_result`。
...
5. 对于 Technical / Market，日频数据过期通常触发 `flag` 或 `need_more_data`。此处 `flag` 为 Decision Trace 注记标记（非 Constraint `status` 枚举值），仅进入 Decision Trace 展示，不修改 Constraint status。此处的 `flag` 与 §12.3 `as_of_mismatch_policy` 中的 `note` 为同层概念（均为 Decision Trace 注记，不改变 status）。
...
> **`time_horizon_mismatch` 实现说明（v0.2.4）：** 该冲突类型比较用户目标周期与 Playbook `time_horizon_bucket`，需访问用户输入参数（非 Analysis Card `constraint_exports`）。MVP 阶段由编排器在调用 Conflict Detection 时传入用户目标周期参数；具体的 condition schema 由 SPEC-007 Orchestration 定义。
...
SPEC-006 v0.2.8 定义了 Investment Playbook 的核心结构、执行语义与 MVP 默认 Playbook。
