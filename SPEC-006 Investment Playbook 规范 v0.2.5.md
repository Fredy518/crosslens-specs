...
> **`partially_applicable` MVP 语义（v0.2.4）：** `partially_applicable` 在 MVP 阶段等同于 `applicable`，不改变 `overall_result` 或 `recommended_decision_bounds`，仅在 Decision Trace 中标注适用性警告。完整行为由 SPEC-011 Playbook Applicability Evaluator 定义。
