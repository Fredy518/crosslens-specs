...
## 11. Playbook Constraint

Playbook Constraint 是 Investment Playbook 中可被系统执行或解释的条件。

MVP 阶段不得仅用字符串列表表示 Playbook 约束，必须使用最小结构化格式。
...
1. Market Regime = tightening；
2. Industry Cycle = early_recovery；
3. Sentiment State = overheated。
...
Validation Report 是 Evaluator 对 Analysis Cards 或 Decision Context 的质量检查结果。

MVP 阶段 Evaluator 不自动重写，但必须有处理闭环。

### 13.1 两阶段 Validation

v0.3 将 Validation 明确为两阶段。

#### Post-card Validation

在 Analysis Cards 收集后立即执行。
...
Run State 中不应使用单值字段 `validation_report`。
...
```json
{
  "validation_reports": [
    ...
    {
      "validation_report_id": "val_pre_decision_001",
      "validation_stage": "pre_decision_validation"
    }
  ]
}
```
...
Conflict severity 分为：

1. low；
2. medium；
3. high。

### 14.2 severity 初始赋值规则
...
Guardrail Report 用于确保系统不输出越界、不可靠或过度自信的投资建议。
...
Resolved Decision Bounds 是 v0.3 新增对象，用于合并来自 Validation、Conflict、Playbook Evaluation 和 Guardrail 的约束。
...
> 护栏高于验证阻塞；验证阻塞高于 Playbook 建议边界；Playbook 边界高于冲突降级；冲突降级高于风格偏好。
...
```text
Playbook Evaluation: recommended_decision_bounds = [wait, add_to_watchlist]
Guardrail: block_all_decision_candidates_due_to_missing_data
```
...
Decision Candidate 必须显式引用其上游约束对象。
...
5. reduce；
6. add_to_watchlist；
7. avoid_under_this_playbook；
8. hold_if_already_owned。
...
1. 用户想分析一只股票是否值得关注；
2. 用户想按某个 Playbook 判断是否买入；
3. 用户已经持有某只股票，想判断是否继续持有；
4. 用户想在下跌后判断是否值得买入。
...
### 23.2 Step 7：Post-card Validation

Post-card Validation 检查 Analysis Card 的字段完整性、证据引用、数据质量、domain_status 和无证据判断。

### 23.3 Step 8：Detect Cross-domain Conflicts

Conflict Detection 在 Post-card Validation 后执行。

Post-card Validation 若发现 Block 级问题，相关 Analysis Card 不应参与 Conflict Detection，或应被标记为低可信输入。

### 23.4 Step 9：Pre-decision Validation

Pre-decision Validation 在 Conflict Detection 后执行，因此 Evaluator 可以感知 Conflict Report。

它主要检查：

1. 高严重度冲突是否被充分暴露；
2. 是否达到最小 Analysis Card 阈值；
3. 是否应进入 analysis_incomplete；
4. 是否需要人工复核。

### 23.5 Step 12：Resolve Decision Bounds

Resolve Decision Bounds 用于合并来自 Validation、Conflict、Playbook Evaluation 和 Guardrail 的约束。

Decision Candidate 只能在 Resolved Decision Bounds 的 allowed_actions 内选择 suggested_action。
...
1. 至少 3/5 个能力域返回 `domain_status = completed` 或 `partial` 的 Analysis Card；
2. Fundamentals Card 必须可用；
3. 至少一个非 Fundamentals 的能力域提供有效支持或反方证据；
4. 不存在 Block 级 Validation Finding；
5. Playbook 的关键 Hard Constraint 可以被判断。
...
### 27.2 Guardrail Checker

Guardrail Checker 接入点：

1. Decision Candidate 生成前；
2. Decision Candidate 生成后。
...
## 28. Observability 与 Decision Trace 的数据关系

系统底层维护统一 Event Log。

Event Log 向上生成两个视图。

### 28.1 Observability View
...
### 29.3 Macro / Meso Evidence 分类

Macro / Meso Evidence 必须标记来源类型：
...
对应能力域应返回 `domain_status = insufficient_data`，并说明：
...
2. 输出 `run_status = analysis_incomplete`；
3. 展示缺失能力域；
4. 提示用户是否转为专项分析。
...
v0.3 的核心变化是把 v0.2.1 中仍然隐含的执行语义显式化。

关键修订包括：

1. 七层架构是职责分层，不是时序执行图；
2. Validation 拆成 Post-card 和 Pre-decision 两阶段；
3. Workflow 新增 Resolve Decision Bounds；
4. Analysis Card 增加 `domain_status`；
5. Playbook Constraint `input_refs` 引用 Evidence Value 或 Metric Registry；
6. Decision Candidate 显式绑定 Playbook Evaluation、Guardrail、Validation 和 Conflict；
7. Guardrail 约束优先级高于 Playbook Evaluation；
8. 能力域之间不得直接相互调用或传递中间结果；
9. 系统状态 `analysis_incomplete` 与建议动作 `need_more_data` 分离；
10. Hard Constraint 引用 Structured Evidence 时必须继承上游不确定性。

该架构的核心价值是：

> 让投研判断从“LLM 直接给结论”升级为“证据生成、能力域分析、质量检查、冲突暴露、Playbook 约束、护栏检查、决策边界解析和决策链展示”的可审计工作流。

crosslens SPEC-003 v0.3 的架构宪法是：

> **Deterministic first, Agentic when necessary, Traceable always.**

---

文档结束 | 版本 v0.3 | 项目 crosslens
