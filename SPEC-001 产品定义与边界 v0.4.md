...
MVP 的端到端验证路径以 Full Decision 为准。产品化运行时应支持三档执行深度：

```text
depth = quick：Quick Check，只跑相关域，快速判断是否值得继续研究；
depth = standard：Standard Review，跑 2~3 个关键域 + Playbook；
depth = deep：Full Decision，五域全跑 + Conflict + Guardrail + Trace。
```

其中 `quick` 与 `standard` 的细粒度域路由是后续 Orchestration 扩展方向；MVP 使用 `deep` / Full Decision 验证五域隔离、冲突暴露和完整 Trace。
