...
crosslens 的第一阶段产品价值不是承诺提高胜率，而是：
...
1. 信息过载；
2. 理由漂移：进入时基于事件，持有时改用技术或长期空间自我解释；
3. 证据污染：传闻、事实、解释、计算指标和主观判断混杂；
4. 复盘失真：没有结构化记录，无法判断错误来自事件证伪、估值、情绪、买点还是数据质量；
5. 分析方法不稳定；
6. 决策链不透明；
7. 用户投资风格难以表达为可执行流程；
8. 研究与风控割裂；
...
6. 用户保留最终判断权；
7. 能力域是分析原语，策略是 Playbook 对一个或多个域能力的组合。
...
同时，五个能力域也不是五个默认独立策略。crosslens 区分三个层级：

```text
能力域 Domain：回答一个分析问题。
工作流 Workflow：定义域内或跨域如何处理输入、评分、降级和输出。
策略 Strategy / Playbook：定义在什么条件下允许买、卖、等、减仓、退出或要求人工复核。
```

因此，所有域都应能独立 workflow 化并输出 Analysis Card；但是否成为独立策略，由 Playbook 决定。
...
Playbook 是策略组合层。它不把五个域机械地变成五个“平行策略机器人”，而是按投资方法选择和组合域能力：

```text
Fundamentals：中长期定价和质量/价值判断；
Event Driven：预期重定价和催化剂判断；
Technical / Market：交易时机、价格行为确认和风险控制；
Macro / Meso：仓位、行业、风格和 regime 约束；
Sentiment：拥挤度、叙事扩散和反身性风险。
```
...
- **MVP-0 / core-runtime stabilization**：验证三域 `standard` workflow 的可运行闭环。
- **MVP-1 / full-decision validation**：在补齐 Event Driven 与 Sentiment 后验证五域 `deep` Full Decision。
...
6. 三个已实现分析能力域返回 Analysis Card：Fundamentals、Technical/Market、Macro/Meso；
...
14. 最小 Decision Trace：阶段顺序、关键对象 refs、route 结果、human-review 触发。

MVP-1 在 MVP-0 基础上补齐：

1. Event Driven 与 Sentiment 两个能力域；
2. `depth = deep` Full Decision 五域验证路径；
3. Decision Trace Level 1 和 Level 2；
4. 基础 Event Log。

产品化运行时应支持三档执行深度：

```text
depth = quick：Quick Check，只跑相关域，快速判断是否值得继续研究；
depth = standard：Standard Review，跑 2~3 个关键域 + Playbook，作为 MVP-0 验证主路径；
depth = deep：Full Decision，五域全跑 + Conflict + Guardrail + Trace，作为 MVP-1 验证主路径。
```

其中 `quick` 的细粒度域路由是后续 Orchestration 扩展方向；MVP-0 使用 `standard` 验证当前 core runtime 的三域闭环，MVP-1 使用 `deep` / Full Decision 验证五域隔离、冲突暴露和完整 Trace。
...
crosslens 输出 Decision Candidate，不输出收益承诺，不替代用户最终决策，不直接生成自动交易指令。
...
> crosslens 是一个以 Orchestrator 和状态化 Workflow 为核心的结构化投研与决策辅助系统。

它不是多 Agent 群聊系统，也不是黑箱买卖信号生成器或自动荐股/自动交易系统。
