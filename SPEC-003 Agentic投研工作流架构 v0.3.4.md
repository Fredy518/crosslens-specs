...
### 3.5 域是分析原语，策略是 Playbook 组合

五个分析能力域是分析原语，不是五个默认独立策略。

```text
Domain = 分析问题边界
Domain Workflow = 域内证据过滤、评分、降级和 Analysis Card 生成流程
Strategy / Playbook = 对一个或多个域输出的组合、约束和动作边界
```

能力域可以独立运行并输出 Analysis Card，但不得在域内直接生成最终买卖动作。策略化发生在 Playbook Evaluation、Conflict Detection、Resolved Decision Bounds 和 Decision Candidate Generation 之后。
...
| Event Driven / Catalyst | 预期重定价和催化窗口 |
| Technical / Market | 交易时机、确认和风险控制 |
| Macro / Meso | regime、行业/风格环境和风险预算 |
| Sentiment | 拥挤度、叙事扩散、反身性风险和反方证据 |

Sentiment 在 MVP 中默认是风险/确认层，不应单独支撑高置信方向性结论。
...
域枚举只表示分析能力边界，不表示五个固定策略。编排器可以根据任务和 `depth` 选择部分域，也可以在 `depth = deep` 的 Full Decision 路径调度全部五域。
...
| `depth` | 产品标签 | 目标 | 域调度 |
|---|---|---|---|
| `quick` | Quick Check | 快速判断问题是否值得继续研究 | 只调度与任务直接相关的域 |
...
在 `depth = deep` Full Decision 路径中，生成完整 Decision Candidate 至少满足：
...
4. Context Bundle；
5. Evidence Packet；
6. `depth = deep` 下五个能力域 Analysis Card；
