...
v0.3.0 将关键执行语义从 Markdown 提升为可运行的 Executable Specification。主要补齐：

1. 新增 `executable_specs/spec006` Python 包，作为 `multi_rule`、`overall_result`、动作边界和 `confidence_cap` 合并的规范实现；
2. 新增 Pydantic v2 契约及生成的 JSON Schema，缺失字段、非法枚举和跨字段矛盾在运行前失败；
3. 新增边界测试，明确 `ambiguous_label_ref` 使 Playbook `overall_result = requires_human_review`，但不短路剩余评估；
4. `not_passed_for_add_position` 增加正式决策分支；
5. `partially_passed` 本身不移除 `buy`；仅明确的 Hard `block_*` 收窄动作集合；
6. Hard `fail` 若未映射到 `block_new_position`、`block_add_position` 或 `require_human_review`，MVP 契约直接拒绝；
7. 新增 §46，声明代码、契约、测试、生成 Schema 与解释性 Markdown 的职责和一致性要求。

v0.2.9 在 v0.2.8 基础上关闭状态机不可达分支、跨 SPEC 输入降级、快照审计和轻量枚举歧义。主要补齐：

1. `multi_rule + any` 改为显式优先级状态机：任一 pass 即 pass，明确 fail 与未决状态并存时为 partial，仅全量明确 fail 时为 fail；
2. Soft fail 保留全量审计计数，规则 1 匹配后明确不再评估 caution 分支；
3. `confidence_cap` 允许中间累计值低于 0.5，仅在全部来源合并后触发人工复核；
4. §18.2 增加 `constraint_exports` 字段缺失与版本不兼容 fallback 表；
5. derived fact 生产契约与 boolean `fact_value` 归位至 SPEC-004 v0.2.5，SPEC-006 仅消费；
6. `playbook_snapshot_hash` 增加字段投影、JCS canonicalization、SHA-256 和外部依赖快照规则；
7. MVP Preference 明确为非加性排序，不因同向多重命中增加权重；
8. 移除 `condition_logic = none` 的 MVP 枚举，`flag` 更名为 `dt_note`，并拆分两类 `time_horizon_mismatch` 路径。

v0.2.8 在 v0.2.7 基础上关闭执行阶段归属、决策树 guard、最小可用阈值和默认策略边界问题。主要补齐：

1. `confidence_cap < 0.5` 检查归属 Resolved Decision Bounds 阶段，仅设置最终 `requires_human_review`，不改写 Playbook Evaluation Report 的 `overall_result`；
2. §22.3 将“至少一条 Hard pass”和“所有 Hard 均 not_applicable”拆为显式互斥 guard；
3. §8.4 将“关键 Hard Constraint 可以判断”改为可执行状态集合与检查时机；
4. §4.2 就近声明 Analysis Card 与 Playbook 两类 `invalidating_conditions` 的展示合并规则；
5. `partially_applicable` 在 MVP 中改为保留枚举，系统不得主动产生；
6. Hard Constraint 的 `on_insufficient_data` / `on_stale_data` 缺省值显式设为 `need_more_data`；
7. 默认 Constraint 增加 Schema Validation 自检表；
8. 宏观域缺失按 Optional Domain 的 `continue_with_warning` 处理；仅宏观数据存在且实际形成 `macro_regime_vs_playbook` 冲突时触发人工复核。

v0.2.7 补齐多来源 `confidence_cap` 下限保护、宏观域缺失处理、`invalidating_conditions` 双来源展示、Applicability 周期偏离阈值及开放问题结论。
...
其中 `invalidating_conditions` 不参与当次 Constraint Evaluation。Playbook Evaluation 仅将其连同来源信息转交 Decision Trace；Analysis Card 与 Playbook 顶层的两类失效条件按 §28.3 的“双来源并集不合并”规则展示。
...
Resolved Decision Bounds 负责计算最终 `confidence_cap` 和最终 `requires_human_review`。Playbook Evaluation Report 的 `overall_result` 是上游评估事实，进入 Resolved Decision Bounds 后不得被回写或改名为另一个 `overall_result`。
...
> **合并方向声明（v0.1.6，v0.2.9 修正）：** Playbook Evaluation Report 生成 `recommended_decision_bounds`，由 Resolved Decision Bounds 进一步合并 Conflict、Validation、Guardrail 等约束。后续来源只能进一步收窄 `allowed_actions`（移除动作或降低 confidence），不得恢复已被 Playbook 移除的动作。`confidence_cap` 下调采用叠加语义——各来源的下调值依次累加，取最终结果（例如 Playbook 下调 0.05、Guardrail 再下调 0.05 → 累计下调 0.10），而非取各来源最大下调值。中间值和最终下限检查见 §23.2；完整合并对象由 SPEC-003 和 SPEC-009 定义。
...
> **`conflict_handling` 格式：** 该字段是以 `conflict_type` 为 key、Conflict Handling 规则为 value 的对象，例如 `{"macro_regime_vs_playbook": {...}}`。无规则时使用空对象 `{}`；MVP Schema Validation 不接受数组或 `null`。
...
MVP 阶段默认只使用 `active`。非 `active` 状态（`draft`、`deprecated`、`archived`）的 Playbook 在 MVP 中拒绝执行，规则引擎返回 validation error。开发/测试环境可通过 Run Config 中的 `allow_non_active_playbook = true` 标志绕过此检查（仅限非生产环境），并在 Decision Trace 中标注"非生产状态 Playbook"。环境判别由 Run Config 的 `environment` 字段控制，枚举 `production | staging | development`，`allow_non_active_playbook = true` 仅在非 `production` 时有效。
...
4. 在宏观域存在可用数据时，是否形成显著 `macro_regime_vs_playbook` 冲突（MVP 中通过 Conflict Handling §25 实现，非 Invalidating Conditions）。宏观域缺失本身不等于该冲突；
...
第 1-4 条在 Playbook Evaluation 步骤 3 检查。第 5 条必须在 input refs 解析、freshness 检查和 Hard Constraint 执行完成后检查；实现者不得在步骤 3 仅凭域状态提前推断。第 5 条不满足时，Playbook Evaluation Report 仍应生成，`overall_result = need_more_data`，但不得生成完整 Decision Candidate，且 `run_status = analysis_incomplete`。
...
`condition_logic = none` 不属于 MVP schema。规则引擎在 Schema Validation 阶段遇到该值或其他未知值时，必须返回 validation error；未来若引入该语义，应通过新版本 schema 正式增加，不在当前枚举中预留。

### 13.3 子规则数据不足时的整体判断

> v0.1.7 新增。

子规则先各自计算 `status`，再按以下顺序聚合。`not_applicable` 子规则先从集合中移除；若移除后为空集，整体为 `not_applicable`，不得产生 vacuous pass。

`condition_logic = all`：

1. 任一子规则 `fail` → 整体 `fail`；
2. 否则，任一子规则 `error` → 整体 `error`；
3. 否则，任一子规则 `partial` → 整体 `partial`；
4. 否则，存在 `insufficient_data` → 整体 `insufficient_data`；
5. 否则，存在 `stale_data` → 整体 `stale_data`；
6. 否则，所有剩余子规则均为 `pass` → 整体 `pass`。

`condition_logic = any`：

1. 任一子规则 `pass` → 整体 `pass`，无论同时存在多少 `fail`、`partial`、`insufficient_data` 或 `stale_data`；
2. 无 `pass` 时，任一子规则 `error` → 整体 `error`；
3. 无 `pass` / `error` 时，任一子规则 `partial` → 整体 `partial`；
4. 无 `pass` / `error` / `partial`，且 `fail` 与 `{insufficient_data, stale_data}` 至少一种并存 → 整体 `partial`；未决子规则仍可能使 `any` 成立；
5. 所有可适用子规则均为 `fail` → 整体 `fail`；
6. 仅剩数据缺口状态时，若存在 `insufficient_data` → 整体 `insufficient_data`，否则整体 `stale_data`。

因此，`any` 模式不存在“已有 `pass` 但因其他子规则 mixed 而返回 `partial`”的路径；`pass` 始终短路为整体 `pass`。`partial` 只表示尚无 pass、但至少一个未决子规则仍可能改变整体结果。子规则自身无 `on_insufficient_data` / `on_stale_data` 字段，数据不足和过期的动作影响由顶层 Constraint 统一处理。

此规则与 §17.2、§22.3 规则 8/9 及 §23.2 规则 9 对齐。
...
> **部分解析规则（v0.2.9）：** 若无任何 `input_ref` 可解析，整体为 `insufficient_data`。若部分 ref 可解析，则先判断已知值是否已足以确定最终结果：`any_avoid_value_present` 已出现匹配值时为 `fail`；`all_avoid_values_present` 已收齐全部 avoid values 时为 `fail`；`all_refs_in_avoid_values` 已出现任一不在 avoid values 中的值时为 `pass`。若已知值尚不能确定最终 pass/fail，但至少一个 ref 已成功解析，整体为 `partial`。该状态进入 Decision Trace，且不执行 `on_fail`。
...
`on_stale_data` 为 Soft Constraint 的可选字段。**缺省值为 `note`**（与 `on_insufficient_data` 一致——数据过期视为轻量信息缺口，不影响动作边界）。Soft Constraint 的 `on_stale_data` 字段与 Hard Constraint 的 `on_stale_data` 分离：Hard Constraint 的缺省值由 §20.5 规定为 `need_more_data`（阻断性），Soft Constraint 中默认 `note`（非阻断性）。若 Playbook 需要更强的响应，必须显式声明。
...
|---|---|---|---|
| `action_ranking` | 影响建议动作排序 | 由实现层定义排序权重；步骤 10 应用 | ✅ 是 |
...
> **MVP 非加性语义（v0.2.9）：** `action_ranking` 是对目标动作的幂等排序提升，不是可累加分数。同一动作被一个或多个 Preference 命中时，排序效果相同；所有命中原因仍须分别进入 Decision Trace。不同动作同时被提升时，按 `priority` 与声明顺序解决相对顺序。实现层不得引入未在本 SPEC 声明的隐藏权重、票数或倍增规则。
...
`partial` 表示已有部分有效输入或子结果，但当前 evaluation mode 尚不能确定整体 `pass` / `fail`，且情况不属于纯数据缺失。MVP 的明确触发路径为：

1. `multi_label_avoid` 部分 ref 已解析，但已知值不足以确定最终 pass/fail（§14.2）；
2. `multi_rule + all` 无 `fail` / `error`，但至少一个子规则为 `partial`；
3. `multi_rule + any` 无 `pass` / `error`，但至少一个子规则为 `partial`；
4. `multi_rule + any` 无 `pass` / `error` / 子规则 `partial`，但明确 `fail` 与 `insufficient_data` 或 `stale_data` 并存。

`multi_rule + any` 中只要存在任一 `pass`，整体必须为 `pass`；只有所有可适用子规则均明确 `fail` 时，整体才为 `fail`。完整优先级见 §13.3。
...
规则引擎执行 Constraint 前，必须先验证 Analysis Card 的 `schema_version`，再解析每个 input_ref，并获得：export_ref、export_type、evidence_ref、value、fact_value 或 label_value、determinism_level、can_support_hard_constraint、allowed_constraint_types、data_quality、data_freshness、confidence（若适用）。

如果无法解析，Constraint status = `insufficient_data`。

字段缺失或契约不兼容时按下表保守降级，不得由 LLM 猜测字段含义，也不得通过宽松默认值扩大 export 的可用范围：

| 字段或条件 | fallback | 对 Constraint 的影响 |
|---|---|---|
| `schema_version` 缺失或不受支持 | 拒绝按当前 Schema 解释该 Card；仅允许使用已注册的向后兼容适配器 | 引用该 Card 的 Constraint 为 `insufficient_data`；若涉及 Hard export，Post-card Validation = `block` |
| `export_ref` / `export_type` 缺失或非法 | 该 export 视为不可解析 | `insufficient_data` |
| `evidence_ref` 缺失或不可追溯 | 该 export 无效 | `insufficient_data`；Hard export 同时触发 Post-card Validation `block` |
| `value_path`、value、`fact_value` 或 `label_value` 按对应 export 类型缺失 | 不按名称、存在性或上下文推断值 | `insufficient_data` |
| `determinism_level` 缺失或未知 | 视为 `unknown` | 不得用于 Hard Constraint；Soft Constraint 为 `insufficient_data`，除非其 schema 明确允许 `unknown` |
| `can_support_hard_constraint` 缺失 | 默认为 `false` | Hard Constraint 为 `insufficient_data` |
| `allowed_constraint_types` 缺失或空数组 | 默认为空集合 | 任何 Constraint 均不得消费该 export |
| `data_quality` 缺失 | 视为 `unknown` | Hard Constraint 为 `insufficient_data`；Soft Constraint 继续与否由其缺失策略决定并写入 Decision Trace |
| `data_freshness` 缺失 | 不推断为“新鲜” | Hard-capable export 按 SPEC-004 Post-card Validation 阻断；其余引用进入 `stale_data` 或既定缺失策略 |
| `confidence` 缺失 | 记为 `null` | 不单独导致引用失败；仅当具体规则依赖 confidence 时为 `insufficient_data` |

Analysis Card `schema_version` 格式和生产者质量规则见 SPEC-004 §4、§41。兼容适配器的标识与转换结果必须进入 Decision Trace。

### 18.3 label://event_certainty 多事件解析规则

> v0.1.1 新增。

Event Driven / Catalyst 的 `event_certainty` 是 `event_list[]` 内每个事件的属性，不是全局单值。
...
5. `ambiguous_label_ref` 应导致该 Soft Constraint 或 Conflict Handling 引用直接触发 `requires_human_review`。**此路径独立于 `on_fail`——即使在 §23.2 NOTE 中 `status = partial` 的 Constraint 不执行 `on_fail`，`ambiguous_label_ref` 在解析阶段（步骤 4）即触发人工复核标记**，不依赖 Constraint 评估结果传播。触发后规则引擎应继续执行剩余评估与报告步骤，不短路退出，以确保 Decision Trace 完整，同时在最终 Playbook Evaluation Report 中合并 `requires_human_review = true`。该路径触发的 `requires_human_review` 在 `reasoning` 中的来源标注为 `input_resolution_ambiguous`，区别于 `constraint_fail` 和 `conflict_handling` 路径。
...
> **derived fact 生产者边界（v0.2.9）：** 上述 derived fact 的生产契约、真假判定与缺失规则由 SPEC-004 §26.2 定义。SPEC-006 只消费已通过 `constraint_exports` 导出的 fact，不规定 Event Driven / Catalyst 能力域的内部实现，也不得在缺失时自行派生。未导出或不符合 §18.2 输入契约时，按 `insufficient_data` 处理。
...
Hard Constraint 的 `on_insufficient_data` 和 `on_stale_data` 均为可选字段，**缺省值统一为 `need_more_data`**。缺省行为具有阻断性：对应 Constraint 分别产生 `status = insufficient_data` 或 `status = stale_data`，并按 §23.2 进入 `overall_result = need_more_data` 路径。

Playbook 可以显式声明更严格的人工复核行为，但不得把 Hard Constraint 的缺省值降为 `note` 后静默生成完整 Decision Candidate。
...
> **`on_hard_constraint_fail` 分工（v0.1.3）：** 此映射仅处理 `block_*` 类的动作移除逻辑。`need_more_data` 和 `stale_data` 的处理由 §23.2 聚合规则负责（设置 `overall_result` 而非直接移除动作）。因此 `on_hard_constraint_fail` 中不应包含 `need_more_data` 或 `stale_data` 作为 key。此映射在 MVP 动作集合不含 `strong_buy`/`strong_sell` 的前提下有效；如未来新增此类动作类型，映射表需同步更新。
...
> **`partially_applicable` MVP 语义（v0.2.8）：** `partially_applicable` 是为 SPEC-011 预留的枚举值，MVP 系统不得主动产生。MVP 的 Applicability 检查只能输出 `applicable`、`not_applicable`、`requires_human_review` 或 `unknown`。若系统消费到未来版本或外部组件产生的 `partially_applicable`，应按 `applicable` 继续计算边界，同时写入 Schema Validation warning 和 Decision Trace；不得据此推断新的动作限制。
...
| overall_result | 语义 |
|---|---|
| `passed` | 至少一条 Hard Constraint 实际 `pass`，其余 Hard 为 `pass` 或 `not_applicable`，且 Soft Constraint 无 fail |
| `passed_with_caution` | 规则 1 未匹配、无 Hard `fail` 或 unresolved status，且 `soft_fail_count_total ≥ 2`；允许没有实际 Hard `pass`，但必须在 Decision Trace 标注。`recommended_decision_bounds` 同 `passed`，`confidence_cap` 下调 0.05 |
...
> **`overall_result` 决策树（v0.3.0 Executable Specification 对齐）：** Applicability 已判定为可执行且第 1-4 条域级门控通过后，执行以下 Constraint 决策树；§8.4 第 5 条由 Hard status 检查实现。条件按优先级从高到低匹配，第一个满足即停止对 `overall_result` 的分支判断，但不停止剩余评估与报告生成。`stale_data`、`insufficient_data`、`partial` 和 `error` 均不视为 `pass`。`soft_fail_count_total` 统计全部 `status = fail` 的 Soft Constraint，包括触发人工复核的项。

| 规则 | 条件 | overall_result |
|---|---|---|
| 1 | 任一解析/策略人工复核信号成立，或任一 Constraint fail 的 impact 为 `require_human_review`；包括 `ambiguous_label_ref` | `requires_human_review` |
| 2 | 任一 Hard Constraint fail（`block_new_position`） | `not_passed_for_new_buy` |
| 3 | 任一 Hard Constraint fail（`block_add_position`），且规则 2 未匹配 | `not_passed_for_add_position` |
| 4 | 无阻断性 Hard fail，且任一 Hard status ∈ `{partial, insufficient_data, stale_data, error}` | `need_more_data` |
| 5 | 规则 1 未匹配；**至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 `soft_fail_count_total = 0` | `passed` |
| 6 | 规则 1 未匹配；**至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 `0 < soft_fail_count_total < 2` | `partially_passed` |
| 7 | 规则 1 未匹配；**至少一条 Hard status = `pass`**，其余 Hard status ∈ `{pass, not_applicable}`，且 `soft_fail_count_total ≥ 2` | `passed_with_caution` |
| 8 | 规则 1 未匹配；**所有 Hard status = `not_applicable` 或不存在 Hard Constraint，且无一实际 `pass`**，`soft_fail_count_total < 2` | `partially_passed` |
| 9 | 规则 1 未匹配；**所有 Hard status = `not_applicable` 或不存在 Hard Constraint，且无一实际 `pass`**，`soft_fail_count_total ≥ 2` | `passed_with_caution` |

规则 5 与规则 8/9 的 guard 互斥。规则 8/9 不阻断动作边界，但 Decision Trace 必须说明“本次没有实际 Hard Constraint 被验证”。`partially_passed` 和 `passed_with_caution` 本身不移除 `buy`；动作集合只由明确的 Hard `block_*` 或其他边界来源收窄。

规则 1 命中后，规则 2-9 不再改变 `overall_result`，也不得仅因同一批 Soft fail 数量达到阈值而额外生成 `passed_with_caution`。全量 `soft_fail_count_total` 与各失败明细仍保留在报告中，剩余 Constraint、Conflict Handling 和 Preference 继续执行。

MVP 中 Hard `fail` 的 impact 必须属于 `{block_new_position, block_add_position, require_human_review}`。其他组合是契约错误，不得猜测 `overall_result`。
...
7. 按 §8.4 第 5 条检查所有 Hard Constraint 的最终可判断状态；
8. 执行 Soft Constraints；
9. 应用 Conflict Handling Policy；
10. 应用 Preferences；
11. 生成 `recommended_decision_bounds`；
12. 记录所有被移除动作及原因；
13. 生成 Playbook Evaluation Report。
...
4. 规则 1 未命中时，多个 Soft Constraint fail → 降低 confidence cap。此规则仅在 `soft_fail_count_total ≥ 2` 时触发（与决策树规则 7/9 同步）；`soft_fail_count_total = 1` 仅触发 `partially_passed` 状态，不降低 confidence cap；
5. 规则 1 未命中，至少一条 Hard Constraint 实际 `pass`、其余 Hard 为 `pass` 或 `not_applicable`，且 `soft_fail_count_total ≥ 2` → `overall_result = passed_with_caution`；
6. Conflict Handling 可移除动作或降低 confidence；
7. Preference 只影响动作排序，不得恢复被移除动作；
8. Guardrail 与 Validation 可在 Resolved Decision Bounds 阶段进一步收窄 Playbook 推荐边界或设置最终复核标记，但不得回写 Playbook Evaluation Report 的 `overall_result`；
...
> **`passed_with_caution` 与 `partially_passed` 的边界（v0.1.5，v0.2.8 修正）：** `passed_with_caution` = 无 Hard fail / unresolved 且 Soft Constraint fail ≥ 2；`partially_passed` = 无 Hard fail / unresolved 且 Soft Constraint fail < 2，但不满足 `passed`。所有 Hard 均为 `not_applicable` 时同样使用该 Soft fail 阈值，但永远不得进入 `passed`。两者均为非阻断状态，区别在于 Decision Trace 展示的醒目程度和 `confidence_cap` 的下调幅度。MVP 暂定 Soft Constraint fail 阈值为 2，可由 Run Config 配置。
...
> **`require_human_review` 优先级（v0.1.6，v0.2.9 计数口径明确，v0.3.0 同步约束范围）：** 若任何 Constraint 的 `on_fail` 包含 `require_human_review` 且该 Constraint 实际 fail，`overall_result` 优先设为 `requires_human_review`。该失败仍计入 `soft_fail_count_total`，但规则 1 已短路后续 `passed_with_caution` / `partially_passed` 分支；不存在”从计数中消费”该失败的过程。

> **`confidence_cap` 下调幅度、中间值与阶段归属（v0.2.9）：** `passed_with_caution` 在 Playbook Evaluation Report 的 `confidence_cap_adjustments` 中记录默认绝对值下调 0.05；该幅度与 Soft Constraint fail 阈值（2）均可由 Run Config 配置。Resolved Decision Bounds 按固定来源顺序 `Playbook → Conflict → Validation → Guardrail` 应用调整，每一步记录 `source_ref`、`cap_before`、`adjustment` 和 `cap_after`。中间 `cap_after` 允许低于 0.5，仅作为审计状态，不触发人工复核；数值只在低于 0 时截断为 0，不在 0.5 截断。全部来源合并完成后才执行一次下限检查：若最终 `confidence_cap < 0.5`，必须设置 `resolved_decision_bounds.requires_human_review = true`，并在 `confidence_cap_reason` 记录阈值触发原因。Playbook Evaluation 阶段不检查最终 cap，也不因 cap 下限改写自身 `overall_result`。Decision Trace 同时保留 `playbook_evaluation_report.overall_result`、全部中间累计记录与 `resolved_decision_bounds.requires_human_review`；不得生成第二个或相互矛盾的 `overall_result`。
...
> **Hard fail 与数据缺口优先级：** 明确的 `block_new_position` / `block_add_position` Hard fail 优先于 Hard 数据缺口。若两者并存，保留阻断型 `overall_result`，数据缺口仍进入 Decision Trace。
...
5. 对于 Technical / Market，日频数据过期通常触发 `dt_note` 或 `need_more_data`。`dt_note` 是 Decision Trace 注记动作（非 Constraint `status` 枚举值），仅进入 Decision Trace 展示，不修改 Constraint status；它与 §12.3 `as_of_mismatch_policy` 中的 `note` 为同层概念。v0.2.9 起旧值 `flag` 不再属于本字段的合法值，Schema Validation 必须拒绝。
...
以上五个为 MVP 支持的冲突类型。§26.1 `require_review_on` 中的 `macro_regime_vs_playbook` 引用此表中的值。新增类型需同时更新两处。若编排器产生某冲突类型但 Playbook 中无对应 Conflict Handling 规则，默认按 `note` 处理并进入 Decision Trace（兜底行为）。`require_review_on` 列表的优先级高于 Conflict Handling 兜底 `note`——若冲突类型同时匹配 `require_review_on`，触发人工复核而非兜底 `note`。

> **`time_horizon_mismatch` 路径边界（v0.2.9）：** 本节的 Conflict Handling 仅消费 SPEC-004 §44 定义的**跨能力域周期错配**，比较不同 Analysis Card 的机器可读周期字段；其 condition schema 和编排输入由 SPEC-007 定义。用户目标周期与 Playbook 周期偏离超过 50% 的检查属于 §7.2 Applicability 路径，不经过 Conflict Handling。两条路径可以同时触发，但必须使用不同的 `reasoning_source`（`applicability_horizon_deviation` / `cross_domain_time_horizon_mismatch`）分别进入 Decision Trace，不得互相覆盖。
...
> **`condition input_ref` 降级（v0.1.6，v0.2.3 显式化执行顺序）：** 执行顺序为：(1) 若 condition 包含多个 input_ref（未来扩展），尝试从已解析的 ref 推断整体 condition 结果；若仅有单个 input_ref 且无法解析，直接进入步骤 (2)；(2) 按无条件形式（全量触发 `actions`）执行；(3) 两种情况均须在 Decision Trace 中标注原因。若 `actions` 包含 `require_human_review` 或 `block_*` 类动作，降级时 Decision Trace 中须显式标注"因 condition 输入缺失而全量触发阻断动作"。
...
| 来源 | 触发步骤 | 语义 |
|---|---|---|
| Conflict Handling `prefer_wait` | 步骤 9 | 跨域冲突存在时，偏好等待 |
| Preference `prefer_wait_over_buy_when_valuation_expensive` | 步骤 10 | 风格偏好，估值缺乏安全边际时偏好等待 |

两者可以同时命中：若 Conflict Handling 和 Preference 都偏好 `wait`，Decision Candidate 的 `action_selection_reason` 应记录双重原因，但排序提升仍按 §16.1 的幂等语义只应用一次，不增加额外权重。
...
```json
{
  "human_review_policy": {
    "require_review_on": [
      ...
      "major_event_with_low_certainty"
    ],
...
> **`requires_human_review` 触发后不短路（v0.2.6，v0.2.8 步骤同步）：** 无论是 Playbook Constraint `on_fail`、`require_review_on` 列表匹配、还是 `ambiguous_label_ref` 触发，`requires_human_review = true` 标记后规则引擎不短路退出。剩余的 Soft Constraints、Conflict Handling、Preferences 和报告生成步骤仍须完整执行，以确保 Playbook Evaluation Report 包含完整的 `constraint_results` 和 `reasoning`。
...
`playbook_snapshot_hash` 的输入投影固定为：
...
不参与 hash 的顶层字段为：`playbook_name`、`description`、`created_by`、`created_at`、`updated_at`、`versioning` 和已有 `playbook_snapshot_hash`。被纳入对象内部的字段按完整对象参与计算，包括 Constraint 或 Preference 自身的 `name`、`description`；不得再做实现自定义删减。

计算算法：

1. 按上述投影构造 JSON 对象；
2. 使用 RFC 8785 JSON Canonicalization Scheme（JCS）规范化；对象 key 按规范排序，数组保留声明顺序；
3. 对 UTF-8 字节执行 SHA-256；
4. 输出小写十六进制，格式为 `sha256:<64-hex>`。

SPEC-004 的 Analysis Card Schema、`constraint_exports` 注册表和值域不属于 Playbook 内容，因此不进入 `playbook_snapshot_hash`。它们必须通过 `dependency_snapshot_refs` 单独固定，并与本次执行一起进入 Decision Trace。若实现加载了机器可读的 constraint export registry，`constraint_export_registry_snapshot_hash` 按该 registry 的 JCS UTF-8 字节计算 SHA-256；MVP 若仅使用 SPEC-004 静态定义而没有独立 registry，该字段设为 `null`，但 `analysis_card_schema_version` 仍为必填。外部契约变化不会自动改变 Playbook hash；若 Playbook 为适配该变化而修改执行字段，则必须同时更新 `playbook_version`、`playbook_snapshot_hash` 和 `updated_at`。历史 Decision Trace 必须保留当时使用的 Playbook snapshot 与依赖快照引用。
...
> **资本扩张期例外（v0.1.6，v0.2.8 输入边界）：** 当公司处于资本扩张周期（`growth_capex_flag = true`）时，`fcf_margin_ttm > 0` 子规则跳过。该子规则 status 设为 `not_applicable`，整体 Constraint 按剩余子规则计算——与 `not_applicable` 枚举语义一致（§17.2）。Decision Trace 须记录此例外。该例外会改变 Hard Constraint 的执行结果，因此 `growth_capex_flag` 必须解析为 `metric://growth_capex_flag`，且 export 满足 `determinism_level = computed`、`can_support_hard_constraint = true`、`allowed_constraint_types` 含 `hard`。若该 export 不存在或不满足要求，例外不得触发，FCF 子规则按正常路径执行。
...
### 34.5 默认 Constraint Schema 自检

Playbook 加载时必须对默认 Constraint 执行静态 Schema Validation；运行时仍需按 §18 和 §20 校验实际 `constraint_exports` 元数据。

| constraint_id | 类型 / evaluation_mode | 直接引用类型 | 规则依据 | 静态结果 |
|---|---|---|---|---|
| `fundamental_quality_001` | hard / multi_rule | `metric://` × 2 | §9.1、§11.3、§13、§20.1 | pass |
| `growth_vs_industry_001` | hard / ref_vs_ref | `metric://` × 2 | §9.1、§12、§20.1 | pass |
| `valuation_margin_001` | hard / ref_vs_threshold | `metric://` × 1 | §9.1、§11、§20.1 | pass |
| `macro_environment_001` | soft / multi_label_avoid | `label://` × 2 | §9.2、§14、§15 | pass |
| `sentiment_overheat_001` | soft / ref_vs_threshold | `label://` × 1 | §9.2、§11.3、§15 | pass |
| `technical_overextension_001` | soft / ref_vs_threshold | `metric://` × 1 | §9.2、§11、§15 | pass |
| `event_certainty_001` | soft / ref_vs_threshold | `fact://` × 1 | §9.2、§11.3、§15 | pass |

表中 `pass` 仅表示本 SPEC 示例的字段形态与引用类型合法。三个 Hard Constraint 的所有 metric 在实际加载时仍必须满足 `computed + can_support_hard_constraint + allowed hard`；任一项不满足即为 Schema Validation error。`growth_capex_flag` 是 §33.1 的辅助控制输入，不计入上表的直接 Constraint ref 数量，但必须接受相同的 Hard input 校验。
...
3. Resolve Required Domains（同时检查 Optional Domains 状态；仅检查 §8.4 第 1-4 条。若违反，在此步将 `run_status` 设为 `analysis_incomplete`，不继续执行后续步骤）
4. Resolve input_refs from constraint_exports
5. Check data_freshness
6. Evaluate Hard Constraints
7. Check Hard Constraint determinability（执行 §8.4 第 5 条；若不满足，设置 `run_status = analysis_incomplete` 与 `overall_result = need_more_data`，不生成完整 Decision Candidate，但继续执行步骤 8-11 以生成完整报告）
8. Evaluate Soft Constraints
9. Apply Conflict Handling Policy
10. Apply Preferences
11. Generate Playbook Evaluation Report
```
...
7. Preference 记录（`action_ranking` 类型：步骤 10 应用排序；`confidence_adjustment` 类型：MVP 阶段仅进入 Decision Trace 说明，不执行 confidence cap 修改）；
...
14. §46 Executable Specification、Pydantic 契约、生成 Schema 与边界测试。
...
## 46. Executable Specification

SPEC-006 的规范执行制品位于：

```text
executable_specs/spec006/
├── src/crosslens_spec006/decision_logic.py
├── src/crosslens_spec006/models.py
├── tests/test_decision_logic.py
└── schemas/spec006-contracts.schema.json
```
...
| 制品 | 职责 |
|---|---|
| `decision_logic.py` | 定义 `multi_rule`、`overall_result`、动作边界和 `confidence_cap` 的可执行语义 |
| `models.py` | 用 Pydantic 定义输入输出契约与跨字段不变量 |
| `tests/` | 固定边界案例，防止语义回归 |
| `schemas/` | 由 Pydantic 生成、供跨语言实现消费的 JSON Schema |
| 本文档 | 解释设计意图、权衡、适用范围和跨 SPEC 关系 |

发布要求：

1. `python -m pytest` 必须通过；
2. `python scripts/export_schema.py` 后生成文件必须与仓库一致；
3. 执行逻辑变化必须同时修改测试和本文档的设计说明；
4. 若 Markdown 与 Executable Specification 行为冲突，该版本视为无效，不得由实现者自行选择其一；应修复冲突后再发布；
5. 在通过全部测试的有效版本中，执行函数定义行为，Pydantic 定义契约，Markdown 不重复实现规则。

v0.3.0 固定以下边界答案：

1. `ambiguous_label_ref` 设置 Playbook `overall_result = requires_human_review`，并继续完成剩余评估；
2. `partially_passed` 不自动移除 `buy`；
3. `confidence_cap == 0.5` 不触发最终复核，只有 `< 0.5` 触发；
4. `multi_rule + any` 中 `fail + unresolved` 为 `partial`，因为未决子规则仍可能产生 `pass`。

---

## 47. v0.3.0 总结

SPEC-006 v0.3.0 定义了 Investment Playbook 的核心结构、可执行语义、机器契约与 MVP 默认 Playbook。
