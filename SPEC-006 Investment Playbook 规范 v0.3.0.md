...
> **derived fact 生产者边界（v0.2.9）：** 上述 derived fact 的生产契约、真假判定与缺失规则由 SPEC-004 §26.2 定义。SPEC-006 只消费已通过 `constraint_exports` 导出的 fact，不规定 Company Event / Catalyst 能力域的内部实现，也不得在缺失时自行派生。未导出或不符合 §18.2 输入契约时，按 `insufficient_data` 处理。
