...
| SPEC-008 | SPEC-008 Decision Trace 与 Observability v0.2.md | v0.2 | Draft | 设计 | Trace Store Runtime v1 四层 artifact、v0 兼容读取、inspect/replay CLI、phase-summary Event Log 汇总、数据源 lineage 与 redaction；完整 raw append-only Event Log 仍属后续目标 | — |
...
| SPEC-010 | SPEC-010 MVP 范围与验证指标 v0.1.md | v0.1 | Draft | 需求 | MVP 范围宪法、MVP-0 三域 standard runtime、MVP-0.5 fixture/mock deep golden path、standard-ready / deep-shadow-ready 真实数据 gate、MVP-1 五域 deep Full Decision 分层、Macro/Meso accepted partial 口径、验证标准、交付清单、exclusion 列表 | — |
...
| SPEC-016 | SPEC-016 Event Driven 域实现规格 v0.1.0.md | v0.1.0 | Review | 实现 | Event Driven Pipeline（事件发现 / 预期差判断 / 可交易性评分 / 受益链条映射）、reviewed source lineage、runtime score payload-only、PIT/as_of 防泄漏 | — |
| SPEC-017 | SPEC-017 Sentiment 域实现规格 v0.1.0.md | v0.1.0 | Draft | 实现 | Sentiment P0 schema fields、soft-only exports、deterministic P0 scoring、fixture/mock baseline + reviewed real structured observations、adapter gap handling、no LLM/no realtime/no trade action | — |
