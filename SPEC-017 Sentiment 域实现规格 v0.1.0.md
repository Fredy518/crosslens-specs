...
P0 目标不是实现完整情绪分析系统，而是让 CrossLens 五域架构闭合，使 `depth=deep` 在 mock/fixture 数据下可以运行 fundamentals、technical_market、macro_meso、event_driven、sentiment 五张卡，并继续进入 conflict / playbook / governance / bounds / trace 阶段。
...
P0 只消费已经结构化的 mock / fixture evidence，不接实时数据源。
