...
3. **MVP-1 / full-decision validation**：以真实数据源替换 fixture/mock golden path，并验证五域 Full Decision、完整四层 Decision Trace 和基础 Event Log。

除非特别标注，本文中“当前 runtime MVP”指 MVP-0；“deep golden path”指 MVP-0.5；“完整产品 MVP”或“Full Decision MVP”指 MVP-1。
...
MVP-0 选择 `depth = standard` 是为了先把可运行的投研决策辅助闭环做稳定；MVP-0.5 用 `depth = deep` 验证五域 runtime 与报告链路；MVP-1 再以真实数据和完整 Trace/Event Log 验证能力域隔离、跨域冲突、Playbook、Guardrail 和 Decision Trace 的完整链路。这不表示所有用户问题都必须走 Full Decision。
...
| **分析能力域** | MVP-0：3 个已实现域（Fundamentals, Technical/Market, Macro/Meso）；MVP-0.5：fixture/mock 下运行 5 域；MVP-1：真实数据路径补齐 5 个全部（Macro/Meso, Fundamentals, Event Driven, Sentiment, Technical/Market） | 自定义能力域、能力域扩展 |
...
Expected:
  ...
  4. Analysis Cards 生成 → MVP-0 3 张 Card；MVP-0.5/MVP-1 5 张 Card，domain_status 包含 completed/partial
