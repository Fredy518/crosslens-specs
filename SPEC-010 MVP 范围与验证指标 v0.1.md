...
| **分析能力域** | MVP-0：3 个已实现域（Fundamentals, Technical/Market, Macro/Meso）；MVP-0.5：fixture/mock 下运行 5 域；real standard promotion candidate：真实数据三域；real deep shadow：reviewed Event/Sentiment source + AlphaDB 跑 5 域；MVP-1：真实数据路径补齐 5 个全部（Macro/Meso, Fundamentals, Event Driven, Sentiment, Technical/Market） | 自定义能力域、能力域扩展；把 deep shadow gate 直接称为 MVP-1 |
