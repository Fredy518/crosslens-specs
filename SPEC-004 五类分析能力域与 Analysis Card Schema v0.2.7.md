...
v0.2.5 在 v0.2.4 基础上将 Event Driven derived fact 的生产责任正式归入能力域输出契约，并版本化 Analysis Card 输入契约。状态保持 Review。
...
2. **P1** Event Driven 四个 Computed metrics 增加 lineage 约束（§26.1）：必须 `source_event_certainty = confirmed` 且非 Interpreted Evidence；
...
5. **P2** Event Driven 增加 `event_type` 枚举 allowlist（§25.1）。

v0.2 合并 SPEC-004 v0.1 初稿与 v0.1.1 微补丁。主要收紧：

1. Fundamentals 降级规则对齐 SPEC-003 最小分析阈值；
2. Event Driven / Catalyst 的 Hard Constraint 出口收紧；
...
8. ~~关闭 Event Driven 官方公告事件标签支撑 Hard Constraint 的 MVP 开放问题~~ — **v0.2 已关闭：不能**。
...
| industry_cycle_vs_event_driven | 行业下行但存在短期事件催化 | medium |
...
# Part C：Event Driven / Catalyst Domain

## 23. Event Driven / Catalyst 能力域定义

### 23.1 职责

Event Driven / Catalyst 能力域负责识别和解释可能触发目标资产重定价的重大事件与潜在催化。它从策略工作流出发，不限于单一公司的资讯；公司自身事件只是事件驱动域的一类输入。
...
该域的核心问题不是“某家公司发生了什么”，而是“什么事件会通过何种传导路径影响目标股票、同业、行业、主题或市场”。例如：美光业绩显著超预期本身是海外公司事件，但对 A 股存储、半导体设备、AI 算力链可能构成 `cross_market_catalyst`。
...
## 24. Event Driven / Catalyst 输入

可消费的 Evidence Packet 类型包括：

1. `event_driven`；
2. `company_event`；
3. `earnings_release_event`；
4. `guidance_change_event`；
5. `management_change_event`；
6. `product_launch_event`；
7. `regulatory_event`；
8. `litigation_event`；
9. `mna_event`；
10. `supply_chain_event`；
11. `capital_return_event`；
12. `industry_catalyst_event`；
13. `policy_catalyst_event`；
14. `cross_market_catalyst_event`；
15. `commodity_or_supply_shock_event`；
16. `theme_catalyst_event`。
...
## 25. Event Driven / Catalyst domain_payload
...
> v0.2.1 新增；v0.2.7 修订。`event_list[].event_type` 必须使用以下枚举，不允许 Agent 自由发明。`company_*` 与 `industry/policy/cross_market/theme` 不是顶层域，而是 Event Driven 域内的事件子类型。
...
```json
{
  "event_list": [
    {
      "event_type": "cross_market_catalyst",
      "event_scope": "cross_market_catalyst",
      "event_date": "2026-06-14",
      "event_direction": "positive",
      "event_materiality": "high",
      "event_certainty": "confirmed",
      "event_resolution_status": "open",
      "source_entity": "Micron",
      "target_scope": "industry",
      "target_universe": ["A股存储", "半导体设备", "AI算力链"],
      "transmission_path": "海外龙头业绩验证存储景气与AI需求强度",
...
`event_scope` 描述事件来源或催化性质，`target_scope` 描述被影响对象，二者必须与 `transmission_path` 一起使用，避免把外部催化误写成目标公司的自身事件。若事件只发生在目标公司自身，使用 `event_scope = company_specific` 与 `target_scope = single_stock`。

---

## 26. Event Driven / Catalyst constraint_exports

> v0.2 收紧。MVP 阶段事件标签一律不支撑 Hard Constraint。

MVP 阶段，Event Driven / Catalyst 的 Hard Constraint 出口仅限于 Computed Evidence metrics。
...
Event Driven / Catalyst 能力域是以下 derived fact 的生产者。它们必须在 Analysis Card 生成阶段基于结构化 `event_list` 确定性导出；Playbook Evaluation 不得自行重建或猜测这些事实。
...
## 27. Event Driven / Catalyst 常见冲突类型
...
## 28. Event Driven / Catalyst MVP 降级规则
...
3. ~~Event Driven 的官方公告事实能否支撑 Hard Constraint~~ — **v0.2 已关闭：不能**；
...
15. Event Driven / Catalyst 事件标签一律不支撑 Hard Constraint，仅 4 个 Computed metrics 在满足 lineage 约束（source_event 已确认 + 非 Interpreted）时可支撑；
16. Event Driven / Catalyst 负责导出三个仅供 Soft Constraint 使用的事件类 derived facts；
