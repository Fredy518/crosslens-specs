...
Fundamentals 域生产 9 种 Evidence Packet 类型，全部继承自 SPEC-004 §18。
...
|--------|------|------|------|
| `gross_margin_ttm` | float | ratio | TTM 毛利率 |
| `gross_margin_qoq_change` | float | ratio | 毛利率 QoQ 变动 |
| `operating_margin_ttm` | float | ratio | TTM 经营利润率 |
...
| `fcf_yield` | float | ratio | FCF / Market Cap |
...
| 12 | `fcf_yield` | 确定性公式 | ratio | true | cashflow_metrics + valuation_metrics |
...
**公式：**
```
gross_margin_qoq_change = gross_margin_ttm - gross_margin_prev_quarter
其中 gross_margin_prev_quarter = gross_profit[Q_t-1] / revenue[Q_t-1]
正值表示毛利率环比改善
```
...
```json
{
  ...
  "description": "TTM FCF / 市值。衡量以当前价格买入时，公司产生自由现金流的回报率。",
  "value_type": "number",
  "unit": "ratio",
  "metric_category": "computed",
  "source_domain": "fundamentals",
  ...
  "tags": ["valuation", "cashflow", "yield", "fundamentals", "hard_constraint"]
}
```
...
// boolean，不是 ratio
// derived_from_macro_label 检查：此 flag 的计算是否仅用了财务数据（income + cashflow）
// 若需要 Macro/Meso 的 capex_cycle_stage label 才能确定 → 必须返回 false
```
...
function run_fundamentals_domain(job: AnalysisDomainJob) -> AnalysisCard:
    ...
    // ═══ Step 4: Evidence Evidence Packet Assembly ═══
    // Assemble Evidence Packets with confidence per §3.2
    evidence_packets = assemble_evidence_packets(
        job, computed, structured_labels, confidence_overrides
    )
    ...
    confidence = compute_domain_confidence(evidence_packets, structured_labels)
    ...
    card = build_analysis_card(
        ...
        supporting_evidence=filter_positive(evidence_packets),
        opposing_evidence=filter_negative(evidence_packets),
        constraint_exports=build_exports(computed, evidence_packets),
        domain_payload=build_payload(structured_labels, computed),
        data_freshness=compute_freshness(evidence_packets),
...
| Step 4: Evidence Assembly | computed | computed |
...
```
stance = "unavailable"
domain_status = max(domain_status, "partial")
```
...
超出 SPEC-004 §41 通用规则的域级检查：

| # | 检查项 | 触发条件 | 严重度 | 处理 |
|---|---|---|---|---|
| 1 | 财报数据过期 > 90 天 | oldest_evidence_as_of > 90 days ago | flag | 降低 confidence，加入 warnings |
| 2 | 财报数据过期 > 180 天 | oldest_evidence_as_of > 180 days ago | block | domain_status = partial，加入 warnings |
...
| 6 | growth_capex_flag 被 macro label 推断 | growth_capex_flag=true 且 derivation 引用了 capex_cycle_stage | block | growth_capex_flag 强制设为 false，加入 warnings |
...
| 边界测试 | 数据缺失 | 正确 domain_status=partial + reason_code | pytest |
