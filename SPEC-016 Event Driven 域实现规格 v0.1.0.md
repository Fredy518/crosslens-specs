...
6. 真实 deep shadow 只接受 reviewed Event source JSON/JSONL；必须通过 schema / lineage 校验，且不得把 fixture/mock event 当作真实 source event。
...
**A 股默认口径：** Event Driven 域的目标映射默认落在 A 股普通股代码体系（`ts_code` / `.SH` `.SZ` `.BJ`）；事件时间归并、窗口切分、事件后收益和“是否仍可交易”判断均以 A 股交易日历为准。若 source event 来自海外市场，必须先完成时区归一化和 `a_share_effective_trade_date` 推导，再进入后续评分与导出。

**交易实现约束：** A 股的涨跌停、停牌、T+1、公告时点与板块制度差异会直接影响 `pricing_stage`、`tradability_score` 和 post-event reaction 的解释。运行时必须区分“逻辑成立”与“当前仍可交易”，不得把封板、停牌或休市期间的价格缺失误解释为事件无效。
...
### 3.1.1 Reviewed real source minimum schema

真实数据 shadow path 的 Event source 可先采用 reviewed news JSON/JSONL，而不是直接依赖外部实时抓取。每条 reviewed source row 至少必须保留：

| 字段 | 要求 |
|---|---|
| `candidate_id` | 稳定 fingerprint，进入 `source_fingerprint` |
| `provider` | reviewed source provider，进入 `source_provider` |
| `title` | 事件候选标题，不得是 fixture/mock 占位文本 |
| `published_at` | source 发布时间；必须不晚于 workflow `as_of` |
| `url` 或 document id | source lineage，进入 `source_url_or_document_id` |

未通过 reviewed source validation 的 row 不得进入真实 Event Driven card。缺 source feed 时，真实 adapter 应输出 explicit gap / unavailable card，不得用 fixture/mock event fallback。
...
事件后的 1D / 3D / 5D 价格反应窗口，必须按 A 股后续真实交易日推进；遇到周末、节假日、停牌或持续无成交时，不得用自然日补齐，也不得把不可交易窗口静默记作 0 收益。
