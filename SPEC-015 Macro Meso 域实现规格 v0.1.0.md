...
| 2 | `liquidity_metrics` | computed | computed | ✅ | Rate/Liquidity/Policy | required |
...
| 5 | `capex_cycle_metrics` | computed | computed | ✅ | Industry/Capex Cycle | required |
...
**必需性裁决（解决上游缺口 #6）：**
- 产出 `partial` card 的最低必需集 = 3 个 required evidence（`macro_rate_metrics`、`industry_cycle_metrics`、`market_regime_label`），分别覆盖三个 Context 各至少一个。
- 缺失 ≥2 个 required → `domain_status = unavailable`。
- 缺失 1 个 required → `domain_status = partial`（预期常态）。
- 全部 required + 全部 optional 齐备 → `domain_status = completed`（少见，宏观数据常有时滞）。
...
function run_macro_meso_domain(job: AnalysisDomainJob) -> AnalysisCard:
    ...
    confidence = compute_macro_meso_confidence(available_evidence, domain_status)  // §7
...
| 6 Confidence Computation | computed | computed | 三因子公式（§7） | ✅ |
| 7 Card Assembly | computed | computed | 字段填充 | ✅ |
| 8 Domain Validation | computed | computed | 规则检查（§9） | ✅ |
...
三因子模型，与 SPEC-013 §8 同构，但适配 Macro/Meso「partial 是常态」的特性。
...
> 权重与 SPEC-013（0.60/0.25/0.15）略有差异：Macro/Meso 的 evidence 多为 structured/interpreted（置信度低于 fundamentals 的 computed），故降低 evidence 权重、提高 freshness 权重（宏观数据时效性更重要）。
...
- **domain_status_multiplier**：completed→1.0，partial→**0.90**（非 0.80），error/unavailable→0.0。

### 7.3 最终截断

data_quality → confidence_cap（SPEC-004 §5.3 + SPEC-009 通用规则）：

| data_quality | confidence_cap |
|---|---|
| high | 0.85 |
| medium | 0.70 |
| low | 0.55 |
...
> 与 SPEC-013 §8.3 的差异：`low` cap 0.45→**0.55**。因 Macro/Meso 长期 low/partial 是预期（SPEC-004 §16 行 1018），0.45 过于惩罚性。
...
> **关键缺口：** 现有 `CrossLensDataAdapter` Protocol（`crosslens_adapters/base.py:86-233`）**没有任何宏观/中观数据接口**——无利率、收益率曲线、流动性、政策、商品、行业产能、capex 的获取方法。5 个 P0 macro metric 要求 `determinism_level=computed` + `can_support_hard_constraint=true`，必须可计算获取。本节列出需新增的 adapter 方法，是 SPEC-015 落地的最大不确定项（AlphaDB 是否有宏观表待确认，见 §14 开放问题）。

### 10.1 新增 Adapter 方法清单

以下方法需添加到 `CrossLensDataAdapter` Protocol（`base.py`）并实现于 `AlphaDBAdapter` / `TinyDataAdapter` / `MockAdapter`：
...
### 10.2 StandardContract 映射

| StandardContract 字段 | AlphaDB 来源（待确认） | tinydata 来源（待确认） | 单位 | 说明 |
|---|---|---|---|---|
| policy_rate | `rawdata.cb_lpr` / `rawdata.cb_mlf` | td 宏观接口 | % | LPR 1Y 优先，无则 MLF |
| nominal_yield_10y | `rawdata.cb_yield_curve` | td 国债收益率 | % | 10 年期国债 |
| nominal_yield_1y | `rawdata.cb_yield_curve` | td 国债收益率 | % | 1 年期国债 |
| cpi_yoy | `rawdata.macro_cpi` | td CPI | % | CPI 同比 |
| dr007 | `rawdata.dr007` | td DR007 | % | 银行间回购 |
| m2_yoy | `rawdata.macro_m2` | td M2 | % | M2 同比 |
| social_financing_yoy | `rawdata.macro_sf` | td 社融 | % | 社融存量同比 |
| capacity_utilization | `rawdata.industry_capacity`（待确认） | — | % | 分行业产能利用率 |
| capex_yoy | `rawdata.industry_capex`（待确认） | — | % | 分行业资本开支同比 |
| input_cost_index | `rawdata.commodity_index` | td 商品指数 | index | 南华工业品 / 行业定制 |

> AlphaDB 表名为**预期占位**，实际表名/可得性需在实现阶段与数据源 owner 确认。tinydata 对宏观数据的支持范围亦待确认。这是 §14 开放问题 #1。
...
1. **AlphaDB 宏观表可得性（最大风险）。** §10.2 的 AlphaDB 表名均为预期占位。需确认 AlphaDB（Tushare 落地库）是否有利率/流动性/行业产能/capex/商品价格表。若无，5 个 P0 metric 无法满足 `computed` + `hard_constraint` 契约，需降级为 structured/interpreted（影响 can_support_hard_constraint）或寻找替代数据源。**这是 SPEC-015 进入代码实现前的最大前置不确定项。**

2. **tinydata 宏观接口范围。** tinydata 是否提供宏观/行业数据 API？若 AlphaDB 无、tinydata 有，可作 fallback；若两者都无，MVP 只能用 Mock。

3. **行业映射口径。** `get_industry_cycle_data(industry_code)` 的 industry_code 用申万一级还是二级？需与 fundamentals 的 `get_industry_peers` 口径一致（现有用申万 SWHY）。
