...
| 6 | `us_real_yield_10y_change_6m` | 差值 | pp | ✅ | global_liquidity_metrics | Rate/Liquidity/Policy（外部） |
| 7 | `cn_us_yield_spread_10y` | 差值 | pp | ✅ | global_liquidity_metrics | Rate/Liquidity/Policy（外部） |
...
**Adapter 字段映射：** akshare `bond_zh_us_rate`（美 10Y）+ `macro_usa_cpi_yoy`（美国 CPI 同比）；AlphaDB 暂无美债/CPI 表（见 §10 缺口）。
...
- `metric://industry_capex_growth_yoy`
- `metric://commodity_input_cost_change`
- `metric://us_real_yield_10y_change_6m`（本 SPEC 新增，全球流动性）
- `metric://cn_us_yield_spread_10y`（本 SPEC 新增，全球流动性）
...
| 5 | completed 但 evidence 不足 | domain_status=completed 但 available_evidence < 7 | flag | 降级 completed→partial |
...
> **全球流动性数据源策略：** AlphaDB 暂无美债/美元指数/北向资金表（见 §10.2 缺口），`get_global_liquidity_data` 的 AlphaDB 实现可能返回部分 null。MVP 建议新增一个基于 **akshare** 的 adapter 实现（或 AlphaDB 内部调 akshare 补数），akshare 接口可得性见 §10.2-global。用户后续可在 AlphaDB 补全球数据表，或由 adapter 直接从 akshare 获取。
...
### 10.2-global 全球流动性数据源映射（akshare 实测）

> 以下 akshare 接口经实测确认（akshare 1.18.64，2026-06-20）。AlphaDB 暂无美债/美元指数/北向资金表，全球流动性数据建议从 akshare 获取（adapter 内部调 akshare，或用户后续在 AlphaDB 补表）。

| StandardContract 字段 | akshare 接口 | akshare 字段 | 单位 | 可得性 | 说明 |
|---|---|---|---|---|---|
| us_treasury_10y | `bond_zh_us_rate` | `美国国债收益率10年` | % | ✅ | 美 10Y 国债收益率 |
| us_treasury_2y | `bond_zh_us_rate` | `美国国债收益率2年` | % | ✅ | 美 2Y，算 10Y-2Y 利差 |
| us_yield_curve_10y_2y | `bond_zh_us_rate` | `美国国债收益率10年-2年` | pp | ✅ | 直接提供 |
| cn_treasury_10y | `bond_zh_us_rate` | `中国国债收益率10年` | % | ✅ | 中 10Y，算中美利差 |
| us_cpi_yoy | `macro_usa_cpi_yoy` | — | % | ✅ | 美国 CPI 同比，算美实际利率 |
| usd_cny | `currency_boc_safe` | `美元` | 100CNY/USD | ✅ | 外汇管理局中间价 |
| northbound_net_inflow_20d | `stock_hsgt_hist_em` | 北向资金历史净流入 | 亿元 | ✅ | 沪深港通，累计 20 日 |

**全球流动性缺口（akshare 也无）：**
- **美元指数（DXY）**：akshare 无直接接口。**免费替代（已调研确认）**：FRED `DTWEXBGS`（名义广义美元指数，日频，2006 至今，免费 API key）或 `DTWEXAFEGS`（发达经济体美元指数）。FRED API 经 `requests` 或 `pandas_datareader` 调用（环境已装 `pandas_datareader`）。若不想引入 FRED key，可用 `usd_cny` 变动 + `currency_boc_safe` 多币种合成近似。MVP 建议 `DTWEXBGS` 为主、合成近似为 fallback。
- **联邦基金利率目标（FOMC 目标区间）**：akshare 无。**免费替代（已调研确认）**：FRED `DFEDTARU`（目标区间上限，日频，当前 active，值 3.75 @ 2026-06-20）+ `DFEDTARL`（下限）+ `DFF`（日频有效利率）。三者均 FRED 免费下载。`us_treasury_2y` 仍可作市场预期代理补充（2Y 反映对 Fed 路径预期）。MVP 建议 `DFEDTARU` 为政策姿态主指标。
