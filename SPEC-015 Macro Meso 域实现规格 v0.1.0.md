...
**全球流动性缺口（akshare 也无）：**
- **美元指数（DXY）**：akshare 无直接接口。替代：用 `usd_cny` 变动 + `currency_boc_safe` 多币种合成，或标记 null。
- **联邦基金利率目标 / FOMC 点阵图**：akshare 无。`us_treasury_2y` 可作政策利率预期代理（2Y 反映市场对 Fed 路径预期），但非直接政策姿态。
- **TED 利差 / FRA-OIS / SOFR**：akshare 无。MVP 不纳入，`global_score` 用可得指标足够。
- **VIX / CBOE 波动率**：akshare 无美股波动率指数（仅有 A 股 QVIX）。VIX 归 Market Regime Context 更合适，不纳入 global_liquidity_metrics。

> **结论：** 2 个 P0 全球流动性 metric（`us_real_yield_10y_change_6m`、`cn_us_yield_spread_10y`）所需数据（美债 10Y、美国 CPI、中债 10Y）akshare 全部可得，**这两个 metric 在 MVP 可落地**。DXY/联邦基金利率缺口不影响 P0 metric 计算，仅影响 `global_score` 的覆盖完整度。
