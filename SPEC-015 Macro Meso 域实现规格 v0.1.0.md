...
```json
{
  ...
  "tags": ["macro", "global", "fed", "real_yield", "hard_constraint"]
}
```
...
```json
{
  ...
  "tags": ["macro", "global", "yield_spread", "cross_border", "hard_constraint"]
}
```
...
- **completeness_factor** = count(available_required + available_optional) / 9（9 = 3 required + 6 optional，含 global_liquidity_metrics）。
...
- `macro_event_summary`（interpreted）不可用时，completeness 分母 9→8。
...
- 7 个 P0 metric 计算与导出（§4；其中 5 个 registered hard-capable + 2 个 proposed soft-only，CR-7 合并后升级）。
...
- **裁决：** 本 SPEC §3.2.9 新增 `global_liquidity_metrics` evidence_type（computed, can_support_hard=true），覆盖美联储政策姿态（美债 10Y/利差）、美元流动性（美实际利率）、跨境传导（中美利差/USD-CNY/北向资金）。§4 新增 2 个 P0 metric：`us_real_yield_10y_change_6m`、`cn_us_yield_spread_10y`（akshare 数据可得，可落地）。
...
| 测试类型 | 覆盖范围 | 目标 | 工具 |
|---|---|---|---|
| 单元测试 - metric 计算 | 7 个 P0 metric 公式（§4；5 registered + 2 proposed） | 边界值 + None 条件 + 降级窗口 | pytest |
