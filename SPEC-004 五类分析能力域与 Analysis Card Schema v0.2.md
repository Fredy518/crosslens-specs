...
6. **P1** Macro/Meso 长期 `partial` 为 MVP 预期行为，不阻塞 Decision Candidate（§16）；
...
**禁止组合：** `completed + unavailable`。若能力域报告 `domain_status = completed`，说明分析已完成——此时不应该声称数据完全不可用。若所有输入数据均为 `unavailable`，应报告 `insufficient_data` 或 `failed`。Post-card Validation 必须将 `completed + unavailable` 标记为 `block`。
...
> **MVP 预期行为（v0.2.3 新增）：** Macro / Meso 在 MVP 阶段长期处于 `partial` 或 `data_quality = low/medium` 是预期行为，并非系统故障。此时能力域不应阻塞 Decision Candidate 生成（与 §22 Fundamentals 的 `insufficient_data → analysis_incomplete` 逻辑相反）。若 Macro/Meso 的唯一贡献是背景信息，用户可在 Decision Trace 中看到该能力域的输出质量标记，但不影响 Playbook Evaluation 主流程。
...
>**已知局限（v0.2.3 新增）：** 当前 lineage 约束仅检查 source event 的直接 Evidence Packet。若 source event 本身引用了一个 Interpreted Evidence 作为其输入（例如，一个"结构化"事件标签是由 LLM 解释步骤生成的），SPEC-004 层面无法检测这种间接污染。完整的 lineage 递归检查应交由 SPEC-009 Governance 层实现。
...
7. 若 `stance ∈ {positive, moderately_positive, mixed, negative, moderately_negative}`，必须包含 opposing_evidence；
...
**触发条件（v0.2.3 精确化）：** 以下任一条件满足时，标记 `time_horizon_mismatch`：

1. 两个能力域的 `time_horizon_bucket` 相差 ≥ 2 个等级（例如 `short_term` vs `long_term`）；
2. |A 域 `time_horizon_days_max` − B 域 `time_horizon_days_min`| > 90 天，且其中一个域信号周期远短于 Playbook 目标周期。
