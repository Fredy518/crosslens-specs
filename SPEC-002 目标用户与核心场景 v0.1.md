...
└──────────────────────────────────────────┘
    ...
    │   │   └── Decision Candidate: HOLD / REDUCE / ADD_TO_WATCHLIST / NEED_MORE_DATA
    │   │
    │   └── NO → 继续路由
    │
...
| **期望产出** | 1. 持仓状态仪表盘（每只股票的状态灯 + 变化信号摘要）；2. 异常股票详细分析（为什么触发警告，哪些条件被打破）；3. 建议动作（持有/减仓/加入观察列表/需要更多数据 + 置信度）；4. 下次复盘前需要关注的风险点 |
