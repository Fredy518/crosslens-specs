...
**实现参考：** SPEC-013 v0.2.1（消费者域 Pipeline 模式、Evidence 边界、Adapter 映射；非规范性上游）
**上游契约：**
...
- **域语义 vs Registry 分治**：当 SPEC-004 的域语义与 SPEC-005 的 Registry 条目不一致时——`domain_payload` 语义（枚举值域、字段含义）暂按 SPEC-004；`constraint_exports` / `metric://` / `fact://` / `label://` 的注册状态、解析、hard/soft 资格必须按 SPEC-005 / Registry。差异进入 CR，不在 SPEC-015 内直接提升为 registered 或 hard-capable。
...
A 股市场的宏观/中观适配要点：
- 利率环境以中国国债收益率（10Y / 1Y）+ DR007（银行间流动性）为主，**但须叠加美债 10Y 与中美利差**——美联储是全球流动性总阀门，美债 10Y 是全球资产定价锚，中美利差倒挂引发资本外流压力（见 §3.2.9 `global_liquidity_metrics`）。
- 政策环境关注央行货币政策（LPR / MLF / 降准）、产业政策（行业支持/限制目录）；外部关注美联储政策姿态（联邦基金利率/缩表节奏，akshare 无直接接口，用美债 2Y 作预期代理）。
- 行业周期以申万一级/二级行业分类为基准（与 fundamentals 的 `get_industry_peers` 同口径）。
- 商品/原材料成本关注对 A 股影响大的品种（铜、铝、螺纹钢、原油、动力煤）。
- 跨境资金面关注北向资金（沪深港通）净流入——外资行为是美联储流动性传导 A 股的直接观测口。
...
SPEC-004 §12 行 818–827 列出 8 种 evidence_type。本 SPEC 补充 required/optional 标注（解决上游缺口——SPEC-004 §12 未声明必需/可选），并**新增第 9 种 `global_liquidity_metrics`**（覆盖 SPEC-004 §11.1 行 783「汇率、能源、原材料等外部变量」中未落地的全球流动性维度——美联储作为全球流动性总阀门对 A 股的传导）：
...
| 9 | `global_liquidity_metrics` | computed | computed | ❌ | Rate/Liquidity/Policy（外部维度） | optional |

> **`global_liquidity_metrics` 归属说明：** 美联储/美元流动性虽是外部变量，但它直接传导到 A 股的利率与流动性环境（外资行为、中美利差、汇率压力），故归入 Rate/Liquidity/Policy Context 作为**外部维度**，不单列第四 Context。标 optional 因数据源（akshare）部分可得、部分缺（见 §10），缺失时不阻塞 partial card。

**必需性裁决（解决上游缺口 #6）：**
- 3 个 required evidence（`macro_rate_metrics`、`industry_cycle_metrics`、`market_regime_label`），分别覆盖三个 Context 各至少一个。其余 6 个为 optional。
- 缺失 ≥2 个 required → `domain_status = unavailable`。
- 缺失 1 个 required → `domain_status = partial`（预期常态）。
- 3 个 required 全在 → 至少 `partial`；若 optional 齐备度 ≥5/6（即 ≥5 个 optional 可用）→ `completed`（少见，宏观数据常有时滞）。
...
#### 3.2.9 global_liquidity_metrics

```text
evidence_type: global_liquidity_metrics
generation_type: computed
determinism_level: computed
can_support_hard_constraint: false   # 本 SPEC 新增 evidence，其 metric 为 proposed/soft-only（见 §4.1/§8.2），CR-7 合并后可升级
数据来源: Adapter.get_global_liquidity_data()（§10，待新增）/ akshare
获取频率: 日频
```

> **定位：** 美联储是全球流动性总阀门。本 evidence 量化美联储政策姿态与美元流动性对 A 股的传导，覆盖三层：
> 1. **美联储政策姿态**：美债 10Y 收益率（全球资产定价锚）、美债 10Y-2Y 利差（衰退信号）；
> 2. **美元流动性市场**：美债实际利率（10Y TIPS 近似 = 名义 - 美国 CPI）、美元指数（缺口，见 §10）；
> 3. **跨境传导**：中美 10Y 利差（倒挂→资本外流压力）、USD/CNY 汇率、北向资金净流入。

**metrics 字段：**

| metric | 类型 | 单位 | 说明 |
|---|---|---|---|
| us_treasury_10y | number | % | 美 10Y 国债收益率 |
| us_treasury_10y_change_6m | number | pp | 美 10Y 6 个月变动 |
| us_yield_curve_10y_2y | number | pp | 美债 10Y-2Y 利差（负值=倒挂，衰退信号） |
| us_real_yield_10y | number | % | 美实际利率（10Y 名义 - 美国 CPI 同比） |
| us_real_yield_10y_change_6m | number | pp | 美实际利率 6 个月变动 |
| cn_us_yield_spread_10y | number | pp | 中美 10Y 利差（中-美；负值=倒挂） |
| usd_cny | number | — | USD/CNY 中间价 |
| usd_cny_change_6m | number | % | USD/CNY 6 个月变动（正值=人民币贬值） |
| northbound_net_inflow_20d | number | 亿元 | 北向资金 20 日累计净流入 |

**confidence 赋值规则：** computed 默认 1.0；数据 >30 天 ×0.90；DXY 缺失时 usd_cny 衍生代理 ×0.80。
...
| # | metric_id | 公式类型 | 单位 | can_support_hard | registration_status | 来源 Evidence | Context |
|---|---|---|---|:---:|---|---|---|
| 1 | `policy_rate_change_6m` | 差值 | pp | ✅ | registered | macro_rate_metrics | Rate/Liquidity/Policy |
| 2 | `real_yield_change_6m` | 差值 | pp | ✅ | registered | macro_rate_metrics | Rate/Liquidity/Policy |
| 3 | `industry_capacity_utilization` | 直接取值 | % | ✅ | registered | industry_cycle_metrics | Industry/Capex Cycle |
| 4 | `industry_capex_growth_yoy` | 同比 | % | ✅ | registered | capex_cycle_metrics | Industry/Capex Cycle |
| 5 | `commodity_input_cost_change` | 变动率 | % | ✅ | registered | commodity_or_input_cost_metrics | Industry/Capex Cycle |
| 6 | `us_real_yield_10y_change_6m` | 差值 | pp | ❌ | **proposed** | global_liquidity_metrics | Rate/Liquidity/Policy（外部） |
| 7 | `cn_us_yield_spread_10y` | 差值 | pp | ❌ | **proposed** | global_liquidity_metrics | Rate/Liquidity/Policy（外部） |

> **registration_status 裁决（P0 契约合规）：** metric #1–5 是上游 SPEC-004 §14 / SPEC-005 §11.3 已定义的 registered hard-capable export。metric #6–7 是本 SPEC 新增，上游尚未注册——按 SPEC-004 §9.2 规则 8（行 730–734），`proposed` 状态强制 `can_support_hard_constraint=false` + `allowed_constraint_types=["soft"]`。CR-7 合并（SPEC-005 §11.3 补注册）后可升为 registered + hard-capable。本 SPEC 不在 CR 合并前把新增 metric 当作 registered hard export，遵守 §1「不修改上游 WHAT 契约」原则。
...
#### metric://us_real_yield_10y_change_6m

**公式：**
```
us_real_yield_10y_change_6m = us_real_yield_10y[t] - us_real_yield_10y[t-126]
其中 us_real_yield_10y = us_treasury_10y - us_cpi_yoy
正值=美实际利率上行（压制 A 股成长股估值、引发外资流出），负值=下行
若美债或美国 CPI 缺失 → us_real_yield_10y = null → 本 metric = null
历史不足 126 日 → 降级为 ≥63 日窗口，<63 日 → null
```

**输入数据：** `global_liquidity_metrics.us_real_yield_10y`，需 ≥126 日历日

**Adapter 字段映射：** AlphaDB `macro_bond_rate`(country=US, term=10y)（美 10Y，已有）+ akshare `macro_usa_cpi_yoy`（美国 CPI 同比，AlphaDB 待补，见 §10 缺口）。

**Metric Registry 条目：**
```json
{
  "metric_id": "us_real_yield_10y_change_6m",
  "display_name": "6个月美实际利率变动",
  "description": "美10Y实际利率（名义-美国CPI）6个月变动。美实际利率是全球资产定价锚，上行压制A股成长股估值、引发外资流出。",
  "value_type": "number",
  "unit": "percentage_point",
  "metric_category": "computed",
  "source_domain": "macro_meso",
  "producing_package": "pkg_macro_metrics_v1",
  "producing_capability": "cap_global_liquidity_compute",
  "evidence_type": "global_liquidity_metrics",
  "generation_type": "computed",
  "determinism_level": "computed",
  "can_support_hard_constraint": false,
  "registration_status": "proposed",
  "evidence_value_path": "us_real_yield_10y_change_6m",
  "expected_export_ref": "metric://us_real_yield_10y_change_6m",
  "freshness_requirement": {
    "update_frequency": "daily",
    "staleness_threshold_days": 30,
    "valid_until_rule": "next_trading_day_plus_30_days",
    "description": "美债日频+CPI月频，30天内有效"
  },
  "confidence_metadata": {
    "determination_type": "computed_default",
    "default_confidence": 1.0,
  ...
  "tags": ["macro", "global", "fed", "real_yield", "soft_constraint", "proposed"]
}
```

#### metric://cn_us_yield_spread_10y

**公式：**
```
cn_us_yield_spread_10y = cn_treasury_10y - us_treasury_10y
正值=中美利差为正（资本流入压力小），负值=倒挂（资本外流压力）
若中债或美债缺失 → null
```

**输入数据：** `global_liquidity_metrics.cn_us_yield_spread_10y`（由 adapter 用中债 10Y - 美债 10Y 计算）

**Adapter 字段映射：** akshare `bond_zh_us_rate`（同时含中、美 10Y）；AlphaDB `rawdata.macro_yieldcurve`（中债）+ 美债缺口。

**Metric Registry 条目：**
```json
{
  "metric_id": "cn_us_yield_spread_10y",
  "display_name": "中美10年期国债利差",
  "description": "中国10Y国债收益率-美国10Y。负值=倒挂，引发资本外流压力与人民币贬值压力。",
  "value_type": "number",
  "unit": "percentage_point",
  "metric_category": "computed",
  "source_domain": "macro_meso",
  ...
  "can_support_hard_constraint": false,
  "registration_status": "proposed",
  "evidence_value_path": "cn_us_yield_spread_10y",
  "expected_export_ref": "metric://cn_us_yield_spread_10y",
  "freshness_requirement": {
    "update_frequency": "daily",
    "staleness_threshold_days": 30,
    "valid_until_rule": "next_trading_day_plus_30_days",
    "description": "中美国债日频，30天内有效"
  },
  "confidence_metadata": {
    "determination_type": "computed_default",
    "default_confidence": 1.0,
  ...
  "tags": ["macro", "global", "yield_spread", "cross_border", "soft_constraint", "proposed"]
}
```
...
function run_macro_meso_domain(job: AnalysisDomainJob) -> AnalysisCard:
    ...
    optional_evidence_types = [
        ...
        "macro_event_summary", "global_liquidity_metrics"
    ]
    ...
    //   3 required 全在 + optional ≥5/6 → completed
    ...
    elif available_optional_count >= 5:
    ...
    computed = {
        ...
        "us_real_yield_10y_change_6m": extract_or_compute(available_evidence, "global_liquidity_metrics", "us_real_yield_10y_change_6m"),
        "cn_us_yield_spread_10y": extract_or_compute(available_evidence, "global_liquidity_metrics", "cn_us_yield_spread_10y"),
    }
...
`rate_environment` + `liquidity_environment` + `policy_environment` 三者综合，**外加全球流动性外部因子**（`global_liquidity_metrics`）：
...
// ── 全球流动性外部因子（美联储/美元传导）──
// 美实际利率上行 + 中美利差倒挂 + 人民币贬值 = 外部流动性收紧，利空 A 股
// 4 个变量均从 global_liquidity_metrics evidence 提取（§3.2.9 定义），
...
global_score = 0
if us_real_yield_10y_change_6m is not None and us_real_yield_10y_change_6m > 0.5:
    global_score -= 1   // 美实际利率显著上行
if cn_us_yield_spread_10y is not None and cn_us_yield_spread_10y < 0:
    global_score -= 1   // 中美利差倒挂
if usd_cny_change_6m is not None and usd_cny_change_6m > 0.03:
    global_score -= 1   // 人民币显著贬值（>3%）
if northbound_net_inflow_20d is not None and northbound_net_inflow_20d < -50:
    global_score -= 1   // 北向资金大幅净流出
// global_score ∈ [-4, 0]（只减不加：全球流动性收紧是风险，宽松是中性基准）

combined_score = rlp_score + global_score * 0.5   // 外部因子权重 0.5（国内为主）
// combined_score ∈ [-5, 3]

rlp_stance:
  combined_score >= 2  → moderately_positive
  combined_score >= 1  → moderately_positive
  combined_score == 0  → neutral
  combined_score == -1 → moderately_negative
  combined_score <= -2 → moderately_negative
若国内三因子均 unknown → 子 stance = neutral（权重×0.5），global_score 不单独成 stance
```
...
- **completeness_factor** = count(available_in_scope) / 8（分母固定 8 = 3 required + 5 optional_in_scope；`macro_event_summary` 因 interpreted 不计入 completeness，见 §7.4）。
...
### 7.4 MVP 简化 — completeness 分母明确规则

`macro_event_summary` 是 interpreted optional evidence，时效性/可得性不稳定，**不计入 completeness 分母**（避免 interpreted evidence 的缺失拉低 computed/structured 域的 confidence）。`policy_context` 是 structured optional，计入分母。因此：

```text
completeness_factor = count(available_in_scope) / 8
其中 available_in_scope = available_required(3) + available_optional_in_scope
      optional_in_scope = liquidity_metrics + policy_context + capex_cycle_metrics
                        + commodity_or_input_cost_metrics + global_liquidity_metrics  (5 项)
      // macro_event_summary 排除在外（interpreted，不计 completeness）
      // 故分母 = 3 required + 5 optional_in_scope = 8（固定）
```

- `policy_context` 不可用时计入缺失（分母仍 8，分子少 1），同时 rate_liquidity_policy 子 stance 的 policy 分量降权。
- `global_liquidity_metrics` 不可用时计入缺失（分母 8，分子少 1），global_score 全部项不计分。
...
**5 个 Computed metric exports（can_support_hard_constraint=true，registration_status=registered）—— 上游 SPEC-004 §14 已定义：**
- `metric://policy_rate_change_6m`
- `metric://real_yield_change_6m`
- `metric://industry_capacity_utilization`
- `metric://industry_capex_growth_yoy`
...
**2 个 Computed metric exports（can_support_hard_constraint=false，registration_status=proposed，soft-only）—— 本 SPEC 新增，CR-7 合并前不升级为 hard：**
- `metric://us_real_yield_10y_change_6m`（全球流动性）
- `metric://cn_us_yield_spread_10y`（全球流动性）

仅当对应 metric 非 null 时导出；null 时不导出（不导出空值 export）。proposed 状态的 export 遵从 SPEC-004 §9.2 规则 8（行 730–734）：`can_support_hard_constraint=false` + `allowed_constraint_types=["soft"]`。
...
| 5 | completed 但 evidence 不足 | domain_status=completed 但 available_evidence < 8（即 3 required + ≥5 optional 未齐） | flag | 降级 completed→partial |
...
| 8 | 中美利差倒挂 | cn_us_yield_spread_10y < 0 | flag | warnings += "cn_us_yield_inversion"；敏感变量加入 cn_us_yield_spread_10y；若同时人民币贬值>3% → 升级 macro_headwinds |
| 9 | sensitive_variables 缺失 | domain_payload 无 sensitive_variables | note | 补默认值 ["unknown"] |
| 10 | registered metric 无数据源 | registered P0 metric 在 MVP 无可靠数据源 | flag | 不导出该 export；warnings += "missing_metric_source:{metric_id}"；domain_status 维持 partial（见 §10.2-fallback） |
...
> **关键缺口：** 现有 `CrossLensDataAdapter` Protocol（`crosslens_adapters/base.py:86-233`）**没有任何宏观/中观数据接口**——无利率、收益率曲线、流动性、政策、商品、行业产能、capex 的获取方法。上游 SPEC-004 §14 定义的 5 个 registered P0 macro metric 要求 `determinism_level=computed` + `can_support_hard_constraint=true`，必须可计算获取；本 SPEC 新增的 2 个 proposed metric 需 computed 但 soft-only。
>
...
| `get_global_liquidity_data(start_date, end_date)` | `GlobalLiquidityData`（us_treasury_10y, us_treasury_2y, us_cpi_yoy, cn_treasury_10y, usd_cny, northbound_net_inflow_20d） | Rate/Liquidity/Policy（外部） | global_liquidity_metrics |

新增 dataclass：`MacroRateData`、`LiquidityData`、`PolicyEvent`、`IndustryCycleData`、`CommodityData`、`GlobalLiquidityData`（放 `base.py`，与现有 `FinancialStatements` 等并列）。

> **全球流动性数据源策略：** AlphaDB 已有美债/中债收益率（`macro_bond_rate`）、USD/CNY 中间价（`macro_china_rmb_fixing`）、北向资金（`stock_moneyflow_hsgt.north_money`）；缺美国 CPI（akshare 补）、DXY/联邦基金/VIX（FRED/CBOE 补，见 §10.2-global）。`get_global_liquidity_data` 的 AlphaDB 实现可覆盖大部分字段，仅 us_cpi/DXY/fed_rate/VIX 需外部源补数。adapter 内部可先查 AlphaDB，缺失字段 fallback 到 akshare/FRED/CBOE。
...
### 10.2-fallback Registered P0 metric 无可靠数据源时的处理规则

> 上游 SPEC-004 §14 / SPEC-005 §11.3 定义 5 个 registered P0 hard-capable metric，但实测 AlphaDB 缺其中 4 个的数据源（LPR、产能利用率、capex；美国 CPI 缺则连累 real_yield）。进入代码实现前必须裁决：当 registered metric 在 MVP 无可靠数据源时如何处理。本规则确保实现不污染 Metric Registry 语义。

```text
If a registered P0 metric has no reliable data source in MVP:
  1. 不导出该 constraint_export（不导出空值或 null export）；
  2. data_quality.warnings += "missing_metric_source:{metric_id}"；
  3. domain_status 维持 partial（不因单 metric 缺失降为 unavailable，但不得为 completed）；
  4. 不得用代理指标沿用原 metric_id 导出 hard-capable export——若需代理，
     必须用新的 metric_id（如 industry_capex_growth_yoy_proxy_swindex）并单独注册为 proposed/soft-only，
     不得污染原 registered metric 的 Registry 语义；
  5. Decision Trace 记录该 metric 的缺失原因 + 代理（若有）的口径差异。
```

**特别说明 `industry_capex_growth_yoy`：** 若用申万行业指数（`index_swdaily`）作代理，不得沿用 `metric://industry_capex_growth_yoy`——该 metric_id 在 Registry 语义上是"行业资本开支同比"，申万指数是"行业景气度"，口径不同。代理须用新 metric_id（如 `industry_capex_proxy_swindex_yoy`）并标 proposed/soft-only。

### 10.2-global 全球流动性数据源映射（AlphaDB + akshare 实测）

> 以下数据源经实测确认（2026-06-20）。AlphaDB **已有**美债/中债收益率（`macro_bond_rate`）、USD/CNY 中间价（`macro_china_rmb_fixing`）、北向资金（`stock_moneyflow_hsgt.north_money`，覆盖至 2026-06-15）；akshare 补充美国 CPI 等。DXY/联邦基金/VIX 两者均无，用 FRED/CBOE 免费源（见缺口）。

| StandardContract 字段 | AlphaDB 表（优先） | akshare 接口（fallback） | 单位 | 可得性 | 说明 |
|---|---|---|---|---|---|
| us_treasury_10y | `macro_bond_rate`(country=US, term=10y) | `bond_zh_us_rate` | % | ✅ AlphaDB 已有 | 美 10Y 国债收益率 |
| us_treasury_2y | `macro_bond_rate`(country=US, term=2y) | `bond_zh_us_rate` | % | ✅ AlphaDB 已有 | 美 2Y，算 10Y-2Y 利差 |
| us_yield_curve_10y_2y | `macro_bond_rate`(US 10y - US 2y) | `bond_zh_us_rate` | pp | ✅ AlphaDB 已有 | 自算 |
| cn_treasury_10y | `macro_bond_rate`(country=CN, term=10y) | `bond_zh_us_rate` | % | ✅ AlphaDB 已有 | 中 10Y，算中美利差 |
| us_cpi_yoy | ❌ AlphaDB 缺口 | `macro_usa_cpi_yoy` | % | ✅ akshare | 美国 CPI 同比，算美实际利率（AlphaDB 待补，见 plan/alphadb-macro-gap-fill.md #1） |
| usd_cny | `macro_china_rmb_fixing`(pair=美元/人民币) | `currency_boc_safe` | 100CNY/USD | ✅ AlphaDB 已有 | 外汇管理局中间价 |
| northbound_net_inflow_20d | `stock_moneyflow_hsgt.north_money`（累计20日） | `stock_hsgt_hist_em` | 亿元 | ✅ AlphaDB 已有 | 沪深港通北向资金（覆盖至 2026-06-15） |

**全球流动性缺口（AlphaDB + akshare 均无，用 FRED/CBOE 免费源）：**
- **美元指数（DXY）**：AlphaDB `index_global` 无 DXY，akshare 无接口。**免费替代（已调研确认）**：FRED `DTWEXBGS`（名义广义美元指数，日频，2006 至今，免费 API key）或 `DTWEXAFEGS`（发达经济体美元指数）。FRED API 经 `requests` 或 `pandas_datareader` 调用（环境已装 `pandas_datareader`）。若不想引入 FRED key，可用 `usd_cny` 变动 + `currency_boc_safe` 多币种合成近似。MVP 建议 `DTWEXBGS` 为主、合成近似为 fallback。
- **联邦基金利率目标（FOMC 目标区间）**：AlphaDB 无，akshare 无。**免费替代（已调研确认）**：FRED `DFEDTARU`（目标区间上限，日频，当前 active，值 3.75 @ 2026-06-20）+ `DFEDTARL`（下限）+ `DFF`（日频有效利率）。三者均 FRED 免费下载。`us_treasury_2y` 仍可作市场预期代理补充（2Y 反映对 Fed 路径预期）。MVP 建议 `DFEDTARU` 为政策姿态主指标。
...
| 缺口指标 | 数据源 | 接入方式 | 频率 | 环境依赖 | 备注 |
|---|---|---|---|---|---|
| 美元指数 DXY | FRED `DTWEXBGS` | FRED API（`requests`/`pandas_datareader`） | 日频 | 免费 FRED API key | 广义美元指数；`DTWEXAFEGS` 为发达经济体口径 |
| 联邦基金目标利率（上限） | FRED `DFEDTARU` | FRED API | 日频 | 免费 FRED API key | 当前 active；`DFEDTARL` 下限、`DFF` 有效利率同源 |
| VIX 波动率 | CBOE `VIX_History.csv` | `requests` 直连 CSV | 日频 | 无需 key | 1990 至今；`yfinance` `^VIX` 备选 |
| TED 利差（可选） | FRED `TEDRATE` | FRED API | 日频 | 免费 FRED API key | MVP 可选，不纳入 global_score |

> **FRED API key 获取：** https://fred.stlouisfed.org/docs/api/fred/ 注册免费账户即得。建议作为环境变量 `FRED_API_KEY` 注入 adapter，缺失时降级为合成近似（DXY）/null（联邦基金）。CBOE VIX CSV 无需 key，可直接 `requests.get` 下载。
...
- `global_liquidity_metrics`：us_treasury_10y=4.43, us_treasury_2y=4.05, us_cpi_yoy=3.2, us_real_yield_10y≈1.23, cn_treasury_10y=1.73, cn_us_yield_spread_10y=-2.70, usd_cny=7.25, usd_cny_change_6m=0.02, northbound_net_inflow_20d=120.5
...
| global_liquidity_metrics | 日频 | 30 天 |
...
- **MVP 过渡与合并规则：** technical_market 的 `label://market_regime` 导出**暂时保留**（其 regime_metrics 已实现且通过 smoke matrix 111/0）。为避免 Playbook 引用 `label://market_regime` 时的双来源歧义，过渡期采用以下最小优先级规则：
  - Macro/Meso card 的 `label://market_regime` 为 **canonical**（若 `domain_status != unavailable`）。
  - Technical/Market 的 `regime_metrics` 可保留在其 `domain_payload` 内，但**一旦 Macro/Meso card 存在，Technical/Market 不得再导出 `label://market_regime`**（CR-3 解决后强制移除）。
  - 若两者导出冲突值，编排层生成 `ConflictReport(type="market_regime_source_conflict")`，进 Decision Trace。
  - Decision Trace 记录 `market_regime` 的来源域 + 冲突处理。
...
| 测试类型 | 覆盖范围 | 目标 | 工具 |
|---|---|---|---|
| 单元测试 - metric 计算 | 7 个 MVP-deliverable metric 公式（§4；5 registered + 2 proposed） | 边界值 + None 条件 + 降级窗口 | pytest |
...
1. **AlphaDB 宏观表缺口（部分已实测，仍为最大风险）。** §10.2 经 live schema 实测（2026-06-20）确认：`macro_yieldcurve`/`macro_cpi`/`macro_cn_m`/`macro_sf_month`/`macro_pmi`/`macro_shibor`/`macro_bond_rate`/`future_daily`/`index_swdaily`/`stock_moneyflow_hsgt`/`macro_china_rmb_fixing` 可用，但 **DR007、LPR/MLF、美国CPI、分行业产能利用率、分行业 capex、DXY、联邦基金利率、VIX 缺表**（详见 `plan/alphadb-macro-gap-fill.md`）。其中 `industry_capacity_utilization` 与 `industry_capex_growth_yoy` 两个 registered P0 metric 无直接数据源——MVP 可能需 null 或代理降级。若 4 个高优先级缺口（美国CPI/LPR/产能/capex）全不补，5 个 registered metric 仅 3 个可导出（2 个 proposed soft-only 靠 akshare 可算）。**这是 SPEC-015 进入代码实现前必须裁决的数据可得性问题；裁决规则见 §10.2-fallback。**
...
8. **FRED API key 配置。** §10.2-global-2 的 DXY/联邦基金利率/TED 利差依赖 FRED API（免费 key）。需确认 key 作为环境变量 `FRED_API_KEY` 的注入方式，以及缺失时是否降级为合成近似（DXY）/null。CBOE VIX CSV 无需 key，可作无依赖 fallback。
